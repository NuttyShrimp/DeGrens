        ...
        if (ItemData.name !== undefined) {
            if ((ItemData.name).split("_")[0] == "weapon") {
                if (!$("#weapon-attachments").length) {
                    $(".inv-options-list").append('<center><div style="background-color: #059669;" class="inv-option-item" id="weapon-attachments"><p>ATTACHMENTS</p></div></center>');
...
function FormatAttachmentInfo(data) {
    $.post("https://dg-inventory/GetWeaponData", JSON.stringify({
        weapon: data.name,
...
function handleAttachmentDrag() {
    ...
    $(".weapon-attachments-remove").droppable({
        ...
        drop: function (event, ui) {
            $.post('https://dg-inventory/RemoveAttachment', JSON.stringify({
...
$(document).on('click', '#weapon-attachments', function (e) {
    ...
    if (!Inventory.IsWeaponBlocked(ClickedItemData.name)) {
        ...
        $.post('https://dg-inventory/Notify', JSON.stringify({
        ...
        start: function (event, ui) {
            ...
            if (dragAmount == 0) {
                if (itemData.price != null) {
                    ...
                    $(".ui-draggable-dragging").find(".item-slot-amount p").html('(' + amount + ') $' + itemData.price);
                    $(".ui-draggable-dragging").find(".item-slot-key").remove();
                } else {
                    $(this).find(".item-slot-amount p").html('0 (0.0)');
                    $(".ui-draggable-dragging").find(".item-slot-amount p").html(amount + ' (' + ((itemData.weight * amount) / 1000).toFixed(1) + ')');
                    $(".ui-draggable-dragging").find(".item-slot-key").remove();
                }
            } else if (dragAmount > amount) {
                if (itemData.price != null) {
                    $(this).find(".item-slot-amount p").html('(' + amount + ') $' + itemData.price);
                } else {
                    $(this).find(".item-slot-amount p").html(amount + ' (' + ((itemData.weight * amount) / 1000).toFixed(1) + ')');
                }
                ...
                if (itemData.price != null) {
                    ...
                    $(this).find(".item-slot-amount p").html((amount - dragAmount) + ' (' + ((itemData.weight * (amount - dragAmount)) / 1000).toFixed(1) + ')');
    ...
    $("#item-use").droppable({
        ...
        drop: function (event, ui) {
            ...
            if (fromData.useable) {
                ...
                $.post("https://dg-inventory/UseItem", JSON.stringify({
...
function updateweights($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    $(".pro").css("width", per + "%");
    $("#player-inv-weight").html("Gewicht: " + (parseInt(totalWeight) / 1000).toFixed(2) + " / " + (playerMaxWeight / 1000).toFixed(2) + " kg");
    if ($fromInv.attr("data-inventory").split("-")[0] != "itemshop" && $toInv.attr("data-inventory").split("-")[0] != "itemshop") {
        $("#other-inv-label").html(otherLabel)
        $("#other-inv-weight").html("Gewicht: " + (parseInt(totalWeightOther) / 1000).toFixed(2) + " / " + (otherMaxWeight / 1000).toFixed(2) + " kg")
...
$(document).on('click', '.CombineItem', function (e) {
    ...
    if (combineslotData.toData.combinable.anim != null) {
        $.post('https://dg-inventory/combineWithAnim', JSON.stringify({
        ...
        $.post('https://dg-inventory/combineItem', JSON.stringify({
...
function optionSwitch($fromSlot, $toSlot, $fromInv, $toInv, $toAmount, toData, fromData) {
    ...
    $.post("https://dg-inventory/SetInventoryData", JSON.stringify({
...
function SetQualityBar(item, $inv, $slot) {
    if (!Inventory.IsWeaponBlocked(item.name)) {
        if (item.quality == undefined) { item.quality = 100.0; }
        ...
        $inv.find("[data-slot=" + $slot + "]").find(".item-slot-quality-bar").css({
            "width": qualityLabel + "%",
        ...
        if ((toData != undefined || toData != null) && toData.name == fromData.name && fromData.stackable) {
            ...
            if (fromData.amount == $toAmount) {
                if (toData != undefined && toData.combinable != null && isItemAllowed(fromData.name, toData.combinable.accept)) {
                    $.post('https://dg-inventory/getCombineItem', JSON.stringify({ item: toData.combinable.reward }), function (item) {
...
(() => {
    ...
    Inventory.Open = function(data) {
        ...
        $(".pro").css("width", per + "%");
        $("#player-inv-weight").html("Gewicht: " + (totalWeight / 1000).toFixed(2) + " / " + (data.maxweight / 1000).toFixed(2) + " kg");
        ...
        if (data.other != null) {
            ...
            if (name != null && (name.split("-")[0] == "itemshop" || name == "crafting")) {
                ...
                $("#other-inv-label").html(data.other.label)
                $("#other-inv-weight").html("Gewicht: " + (totalWeightOther / 1000).toFixed(2) + " / " + (data.other.maxweight / 1000).toFixed(2) + " kg")
            ...
            $("#other-inv-weight").html("Gewicht: " + (totalWeightOther / 1000).toFixed(2) + " / " + (Inventory.dropmaxweight / 1000).toFixed(2) + " kg")
