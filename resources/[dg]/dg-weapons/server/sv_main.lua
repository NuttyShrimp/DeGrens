        ...
        if Player.PlayerData.items[data.slot].quality then
            if Player.PlayerData.items[data.slot].quality ~= 100 then
                if Player.Functions.RemoveMoney("cash", Config.WeaponRepairCosts[WeaponClass]) then
                    Config.WeaponRepairPoint.IsRepairing = true
                    ...
                    TriggerClientEvent("weapons:client:SyncRepairShops", -1, Config.WeaponRepairPoint)

                    SetTimeout(Timeout, function()
                        Config.WeaponRepairPoint.IsRepairing = false
                        Config.WeaponRepairPoint.RepairingData.Ready = true
                        TriggerClientEvent("weapons:client:SyncRepairShops", -1, Config.WeaponRepairPoint)
                        ...
                        SetTimeout(7 * 60000, function()
                            if Config.WeaponRepairPoint.RepairingData.Ready then
                                Config.WeaponRepairPoint.IsRepairing = false
                                Config.WeaponRepairPoint.RepairingData = {}
                                TriggerClientEvent("weapons:client:SyncRepairShops", -1, Config.WeaponRepairPoint)
                            end
...
RegisterNetEvent("weapons:server:TakeBackWeapon", function(data)
    local src = source
    local Player = DGCore.Functions.GetPlayer(src)
    local itemdata = Config.WeaponRepairPoint.RepairingData.WeaponData
            ...
            if WeaponSlot.quality then
                for i = 1, RepeatAmount, 1 do
                    if WeaponSlot.quality - DecreaseAmount > 0 then
                        ...
                        TriggerClientEvent("weapons:client:UseWeapon", src, data)
...
DGCore.Commands.Add('setammo', 'Set Your Ammo Amount (Admin Only)', {{name='amount', help='Amount of bullets, for example: 20'}}, false, function(source, args)
    local src = source
