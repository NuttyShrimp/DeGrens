    ...
    if GetCurrentResourceName() == resourceName then
        for name, elevator in pairs(Config.Elevators) do
            for levelId, level in pairs(elevator.levels) do
                exports['dg-peek']:RemoveZone(name.."_"..levelId)
            end
        end
    end
end)
...
Citizen.CreateThread(function()
    for name, elevator in pairs(Config.Elevators) do
        for levelId, level in pairs(elevator.levels) do
            exports['dg-peek']:AddCircleZone(name.."_"..levelId, level.interact, 0.3, {
                name = name.."_"..levelId,
                useZ = true,
                debugPoly = GetConvar('is_production', "true") == "false",
            }, {
                options = {
                    {
                        type = "client",
...
AddEventHandler("dg-elevator:UseElevator", function(data)
