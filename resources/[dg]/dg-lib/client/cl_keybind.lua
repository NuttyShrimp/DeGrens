...
exports('GetCurrentKeyMap', function(keycommand, keycontroller)
	...
	local key = GetControlInstructionalButton(keycontroller, GetHashKey(string.format("+keybind_wrapper__%s",keycommand)) | 0x80000000, 1):gsub('t_', ''):gsub('b_2000', '')
	return tostring(key)
end)
...
Citizen.CreateThread(function()
	...
	RegisterCommand('-GeneralUse', function()end, false)
	exports["dg-lib"]:registerKeyMapping('GeneralUse', 'General interaction button', '+GeneralUse', '-GeneralUse', 'e', true)

	RegisterCommand('+housingMain', function() end, false)
	RegisterCommand('-housingMain', function() end, false)
	exports["dg-lib"]:registerKeyMapping("housingMain", "Housing interaction Main", "+housingMain", "-housingMain", "H", true)

	RegisterCommand('+housingSecondary', function() end, false)
	RegisterCommand('-housingSecondary', function() end, false)
	exports["dg-lib"]:registerKeyMapping("housingSecondary", "Housing interaction Secondary", "+housingSecondary", "-housingSecondary", "G", true)
end)
