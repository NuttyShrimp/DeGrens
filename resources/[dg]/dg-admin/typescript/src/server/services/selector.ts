...
const getSelectorCommands = (src: number, objId?: number) => {
  const plyRole = getPlayerRole(src);
  // Array of arrays of actions. (index is entity type)
  const actions: CommandData[][] = [];
  commands
    .filter(
      c => c.target && c.target.length > 0 && hasRoleAccess(plyRole, c.role) && (c.isEnabled?.({ objId }) ?? true)
    )
    .forEach(c => {
...
Events.onNet(
  ...
  (src, data: { name: string; netId: number; entityType: number; objId: number }) => {
    if (!data?.name) return;
    let entity = NetworkGetEntityFromNetworkId(data.netId);
    if (entity && !DoesEntityExist(entity)) return;
    entity = entity || data.netId;
    const cmd = getSelectorCommands(src, data.objId)[data.entityType]?.find(a => a.name === data.name);
    if (!cmd) {
      Util.Log(
...
RPC.register('admin:selector:getActions', (src, objId?: number) => {
  return getSelectorCommands(src, objId).map(a =>
    a.map(c => ({ name: c.name, title: c.UI?.title ?? c.name + ' (bad configured)' }))
  );
});
