...
setPedOnChair = function(ped, chairIdx)
	...
	TaskStartScenarioAtPosition(ped, "PROP_HUMAN_SEAT_CHAIR_UPRIGHT", behindPed.x, behindPed.y, behindPed.z, GetEntityHeading(ped), 0, 1, false)
end

spawnCharPeds = function()
	for k, v in pairs(plyChars) do
	...
	-- Get next free seat
	for i = 1, 6 do
		if entityCache.peds[i] == nil then
			local freeModePed = `mp_m_freemode_01`
		RequestModel(freeModePed)
		while not HasModelLoaded(freeModePed) do
			Citizen.Wait(0)
		end

		local pedChairs = Config.SeatLoc[i]
		local _ped = CreatePed(2, freeModePed, pedChairs.coords.x, pedChairs.coords.y, pedChairs.coords.z, 0.0, false, true)
		SetEntityAlpha(_ped, 51)
		SetEntityInvincible(_ped, true)
		setPedOnChair(_ped, i)
		SetBlockingOfNonTemporaryEvents(_ped, true)
		Entity(_ped).state:set('citizenid', 1)
		entityCache.peds[i] = _ped
			break
		end
	end
end
