...
RegisterNUICallback('selectChar', function(_, cb)
  ...
  local playerData = DGX.RPC.execute('dg-chars:server:loadPly', plyCid)
...
RegisterNUICallback('deleteChar', function(_, cb)
  ...
  DGX.RPC.execute('dg-chars:server:deleteCharacter', entCid)
  DoScreenFadeOut(250)
  Wait(250)
  removeEntities(true)
...
RegisterNUICallback('createChar', function(data, cb)
  ...
  closeCharMenu()
  DGX.RPC.execute('dg-chars:server:createCharacter', data)
  cb({ data = {}, meta = { ok = true, message = 'done' } })
end)
--endregion
--region Spawn
