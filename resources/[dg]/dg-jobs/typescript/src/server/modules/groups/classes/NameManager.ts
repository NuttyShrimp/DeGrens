...
class NameManager extends Util.Singleton<NameManager>() {
  ...
  public async updatePlayerName(player: Core.Characters.Player) {
    const vpnItem = await Inventory.getFirstItemOfName('player', String(player.citizenid), 'vpn'); // this version is more performant if you already have cid
    ...
    if (playerNameData && playerNameData.isVPNName === !!vpnItem) return;

    const name = vpnItem ? Util.generateName(vpnItem.id) : `${player.charinfo.firstname} ${player.charinfo.lastname}`;
    this.logger.debug(`Update name for ${player.name}(${player.citizenid}) to ${name}`);
    this.players.set(player.citizenid, { name, isVPNName: !!vpnItem });
  }
