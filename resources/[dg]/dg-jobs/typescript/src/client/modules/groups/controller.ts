...
// region nui
UI.RegisterUICallback('phone/jobs/groups/get', async (data, cb) => {
  const groups = await DGCore.Functions.TriggerCallback<JobGroup[]>('dg-jobs:server:groups:get');
  cb({ data: groups, meta: { ok: true, message: 'done' } });
});
UI.RegisterUICallback('phone/jobs/groups/create', async (_, cb) => {
  const isSuccess = await DGCore.Functions.TriggerCallback<boolean>('dg-jobs:server:groups:create');
...
UI.RegisterUICallback('phone/jobs/groups/join', async (data: { id: string }, cb) => {
  const isSuccess = await DGCore.Functions.TriggerCallback<boolean>('dg-jobs:server:groups:joinRequest', data);
...
UI.RegisterUICallback('phone/jobs/groups/leave', async (_, cb) => {
  const isSuccess = await DGCore.Functions.TriggerCallback<boolean>('dg-jobs:server:groups:leave');
...
UI.RegisterUICallback('phone/jobs/get', async (_, cb) => {
  const jobs = await DGCore.Functions.TriggerCallback('dg-jobs:server:jobs:get');
  cb({ data: jobs, meta: { ok: true, message: 'done' } });
});
UI.RegisterUICallback('phone/jobs/waypoint', async (data: { job: string }, cb) => {
  const isSuccess = await DGCore.Functions.TriggerCallback<boolean>('dg-jobs:server:jobs:waypoint', data);
...
onNet('dg-jobs:client:groups:isFull', () => {
...
onNet('dg-jobs:client:groups:set', (group: JobGroup | null) => {
  UI.SendAppEvent('phone', {
    appName: 'jobcenter',
...
onNet('dg-jobs:client:groups:setMembers', (members: JobGroupMember[]) => {
  UI.SendAppEvent('phone', {
    appName: 'jobcenter',
    action: 'setMembers',
    data: members,
...
onNet('dg-jobs:client:groups:setGroupOwner', (isOwner: boolean) => {
  UI.SendAppEvent('phone', {
    appName: 'jobcenter',
    action: 'setOwner',
    data: isOwner,
...
onNet('dg-jobs:client:jobs:waypoint', (loc: { x: number; y: number }) => {
  DeleteWaypoint();
