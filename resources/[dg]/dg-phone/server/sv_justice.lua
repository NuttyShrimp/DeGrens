...
RegisterNetEvent('DGCore:Server:OnJobUpdate', function(src, job)
	...
	for _, player in ipairs(registered[job]) do
		if player.srvId == src then
			player.available = job.onduty and Config.justice.availableOnLogin or false
			return
		end
	end
...
RegisterNetEvent('DGCore:Server:OnJobDutyUpdate', function(src, isOnDuty)
	setPlyAvailable(src, isOnDuty and Config.justice.availableOnLogin or false)
end)
