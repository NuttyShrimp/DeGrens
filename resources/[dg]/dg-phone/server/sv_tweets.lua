...
addTweet = function(src, tweet, date)
  ...
  local id = exports['dg-sql']:insert(query, { Player.citizenid, tweet, date })
  tweetCache[id] = {
    id = id,
    cid = cid,
    tweet = tweet,
    date = date,
    sender_name = "@" ..
        ('%s %s'):format(Player.charinfo.firstname, Player.charinfo.lastname):gsub(" ", "_"),
    liked = 0,
    like_count = 0,
    retweeted = 0,
    retweet_count = 0,
...
DGX.RPC.register('dg-phone:server:getTweets', function(src, offset)
  local Player = charModule.getPlayer(src)
  local tweets = getTweets(Player.citizenid, offset)
  return tweets
end)
...
DGX.Events.onNet('dg-phone:server:twitter:addLike', function(src, tweetId)
  ...
  DGX.Util.Log('phone:tweet:addLike', { tweetId = tweetId },
    ('%s has added a like to a tweet'):format(DGX.Util.getName(src)), src)
end)
...
DGX.Events.onNet('dg-phone:server:twitter:removeLike', function(src, tweetId)
  ...
  exports['dg-sql']:query(query, { Player.citizenid, tweetId })
  DGX.Events.emitNet('dg-phone:client:twitter:removeLike', -1, tweetId)

  DGX.Util.Log('phone:tweet:removeLike', { tweetId = tweetId },
    ('%s has removed a like from a tweet'):format(DGX.Util.getName(src)), src)
end)
...
DGX.Events.onNet('dg-phone:server:twitter:addRetweet', function(src, tweetId)
  ...
  DGX.Util.Log('phone:tweet:addRetweet', { tweetId = tweetId },
    ('%s has retweeted a tweet'):format(DGX.Util.getName(src)), src)
end)
...
DGX.Events.onNet('dg-phone:server:twitter:deleteTweet', function(src, tweetId)
  ...
  DGX.Util.Log('phone:tweet:deleteTweet', { tweetId = tweetId }, ('%s has deleted a tweet'):format(DGX.Util.getName(src)),
    src)
end)
...
DGX.Events.onNet('dg-phone:server:twitter:newTweet', function(src, message, date)
  ...
  DGX.Util.Log('phone:tweet:newTweet', { tweetId = insertId, tweetMessage = message },
    ('%s has posted a tweet'):format(DGX.Util.getName(src)), src)
end)