...
RegisterNetEvent('dg-phone:pinger:request', function(data)
  ...
  local Target = charModule.getPlayer(tonumber(data.target))
  ...
  if Target then
    ...
    TriggerClientEvent('dg-phone:pinger:sendRequest', data.target, #pingManager,
      data.isAnon and "Anonymous Number" or Player.charinfo.phone);
  end
end)
