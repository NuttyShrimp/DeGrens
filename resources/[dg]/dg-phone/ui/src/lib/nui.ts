let resourceName = 'dg-phone';

export const setResourceName = (name: string) => {
	resourceName = name;
};

const doRequest = async (action: string, body = {}) => {
	// @ts-ignore
	const rawResult = await fetch(`https://${resourceName}/${action}`, {
		method: 'post',
		headers: {
			'Content-Type': 'application/json; charset=UTF-8',
		},
		body: JSON.stringify(body),
	});
	try {
		return await rawResult.json();
	} catch (err: any) {
		return { data: {}, meta: { ok: false, message: `Failed to do request for: ${action} - ${err.message}` } };
	}
};

export const nuiAction = async (event: string, body = {}, devData: any = {}): Promise<any> => {
	if (process.env.NODE_ENV === 'development') return devData;
	const request = await doRequest(event, body);
	if (!request?.meta?.ok) {
		throw new Error(request.meta.message);
	}
	return request.data;
};

export const emulate = (action: string, app: string, data = null) => {
	window.dispatchEvent(
		new MessageEvent('message', {
			data: {
				action,
				app,
				data,
			},
		})
	);
};

// alternative way to copy instead of navigator.clipboard.writeText
export const copyToClipboard = (str: string) => {
	const el = document.createElement('textarea');
	el.value = str;
	document.body.appendChild(el);
	el.select();
	document.execCommand('copy');
	document.body.removeChild(el);
};
