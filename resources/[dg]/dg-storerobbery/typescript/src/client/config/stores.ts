export const STORE_DATA: Record<Store.Id, Store.Data> = {
  little_seoul: { // DONE
    registerzone: {
      center: { x: -706.17, y: -914.59, z: 19.22 },
      width: 4,
      length: 1.5,
      options: {
        heading: 90,
        minZ: 17.21,
        maxZ: 21.21,
      },
    },
    storezone: {
      center: { x: -711.53, y: -909.74, z: 19.22 },
      width: 14,
      length: 13,
      options: {
        heading: 0,
        minZ: 17.21,
        maxZ: 21.21,
      },
    },
    safecoords: { x: -709.77, y: -904.12, z: 19.22 },
    cam: 1,
  },
  grove_street: { // DONE
    registerzone: {
      center: { x: -47.38, y: -1758.61, z: 29.42 },
      width: 4,
      length: 1.5,
      options: {
        heading: -130.01,
        minZ: 27.42,
        maxZ: 31.42,
      },
    },
    storezone: {
      center: { x: -48.41, y: -1751.49, z: 29.42 },
      width: 14,
      length: 13,
      options: {
        heading: -40.01,
        minZ: 27.42,
        maxZ: 31.42,
      },
    },
    safecoords: { x: -43.43, y: -1748.3, z: 29.42 },
    cam: 2,
  },
  mirror_park: { // DONE
    registerzone: {
      center: { x: 1164.84, y: -323.66, z: 69.21 },
      width: 4,
      length: 1.5,
      options: {
        heading: 100.0,
        minZ: 67.21,
        maxZ: 71.21,
      },
    },
    storezone: {
      center: { x: 1158.68, y: -319.84, z: 69.21 },
      width: 14,
      length: 13,
      options: {
        heading: 10.0,
        minZ: 67.21,
        maxZ: 71.21,
      },
    },
    safecoords: { x: 1159.48, y: -314.0, z: 69.21 },
    cam: 3,
  },
  richman_glen: { // DONE
    registerzone: {
      center: { x: -1819.53, y: 793.5, z: 138.09 },
      width: 4,
      length: 1.5,
      options: {
        heading: 132.46,
        minZ: 136.21,
        maxZ: 140.21,
      },
    },
    storezone: {
      center: { x: -1826.77, y: 793.43, z: 138.21 },
      width: 14,
      length: 13,
      options: {
        heading: 42.46,
        minZ: 136.21,
        maxZ: 140.21,
      },
    },
    safecoords: { x: -1829.22, y: 798.72, z: 138.19 },
    cam: 4,
  },
  grape_seed: { // DONE
    registerzone: {
      center: { x: 1697.29, y: 4923.5, z: 42.06 },
      width: 4,
      length: 1.5,
      options: {
        heading: -215.0,
        minZ: 40.06,
        maxZ: 44.06,
      },
    },
    storezone: {
      center: { x: 1704.34, y: 4925.1, z: 42.06 },
      width: 14,
      length: 13,
      options: {
        heading:  -125.0,
        minZ: 40.06,
        maxZ: 44.06,
      },
    },
    safecoords: { x: 1707.94, y: 4920.44, z: 42.06 },
    cam: 5,
  },
  vespucci_canals: { // DONE
    registerzone: {
      center: { x: -1222.01, y: -908.3, z: 12.33 },
      width: 1,
      length: 1.5,
      options: {
        heading: -146.14,
        minZ: 10.33,
        maxZ: 14.33,
      },
    },
    storezone: {
      center: { x: -1221.45, y: -909.31, z: 12.33 },
      width: 9,
      length: 16,
      options: {
        heading: -146.14,
        minZ: 10.33,
        maxZ: 14.33,
      },
    },
    safecoords: { x: -1220.87, y: -916.05, z: 11.33 },
    cam: 6,
  },
  grand_senora: { // DONE
    registerzone: {
      center: { x: 1165.94, y: 2710.78, z: 38.16 },
      width: 1,
      length: 1.5,
      options: {
        heading: 0,
        minZ: 36.16,
        maxZ: 40.16,
      },
    },
    storezone: {
      center: { x: 1166.05, y: 2711.9, z: 38.16 },
      width: 9,
      length: 16,
      options: {
        heading: 0,
        minZ: 36.16,
        maxZ: 40.16,
      },
    },
    safecoords: { x: 1169.24, y: 2717.82, z: 37.16 },
    cam: 7,
  },
  murrieta_heights: { // DONE
    registerzone: {
      center: { x: 1134.23, y: -982.48, z: 46.42 },
      width: 1,
      length: 1.5,
      options: {
        heading: 97.84,
        minZ: 44.42,
        maxZ: 48.42,
      },
    },
    storezone: {
      center: { x: 1133.11, y: -982.53, z: 46.42 },
      width: 9,
      length: 16,
      options: {
        heading:  97.84,
        minZ: 44.42,
        maxZ: 48.42,
      },
    },
    safecoords: { x: 1126.78, y: -980.15, z: 45.42 },
    cam: 8,
  },
  del_perro: { // DONE
    registerzone: {
      center: { x: -1486.27, y: -378.01, z: 40.16 },
      width: 1,
      length: 1.5,
      options: {
        heading: -45.07,
        minZ: 38.16,
        maxZ: 42.16,
      },
    },
    storezone: {
      center: { x: -1485.42, y: -377.31, z: 40.16 },
      width: 9,
      length: 16,
      options: {
        heading: -45.07,
        minZ: 38.16,
        maxZ: 42.16,
      },
    },
    safecoords: { x: -1478.91, y: -375.45, z: 39.16 },
    cam: 9,
  },
  great_ocean: { // DONE
    registerzone: {
      center: { x: -2966.45, y: 390.89, z: 15.04 },
      width: 1,
      length: 1.5,
      options: {
        heading: -93.60,
        minZ: 13.04,
        maxZ: 17.04,
      },
    },
    storezone: {
      center: { x: -2965.32, y: 390.76, z: 15.04 },
      width: 9,
      length: 16,
      options: {
        heading: -93.60,
        minZ: 13.04,
        maxZ: 17.04,
      },
    },
    safecoords: { x: -2959.66, y: 387.09, z: 14.04 },
    cam: 10,
  },
  banham_canyon: { // DONE
    registerzone: {
      center: { x: -3040.09, y: 584.18, z: 7.91 },
      width: 4.5,
      length: 1.5,
      options: {
        heading: 17.75,
        minZ: 5.91,
        maxZ: 9.91,
      },
    },
    storezone: {
      center: { x: -3044.85, y: 587.65, z: 7.91 },
      width: 12,
      length: 13,
      options: {
        heading: 107.75,
        minZ: 5.91,
        maxZ: 9.91,
      },
    },
    safecoords: { x: -3048.68, y: 588.57, z: 7.91 },
    cam: 11,
  },
  chumash: { // DONE
    registerzone: {
      center: { x: -3243.4, y: 1000.1, z: 12.83 },
      width: 4.5,
      length: 1.5,
      options: {
        heading: 175.07,
        minZ: 10.83,
        maxZ: 14.83,
      },
    },
    storezone: {
      center: { x: -3246.58, y: 1005.06, z: 12.83 },
      width: 12,
      length: 13,
      options: {
        heading: 85.07,
        minZ: 10.83,
        maxZ: 14.83,
      },
    },
    safecoords: { x: -3249.7, y: 1007.37, z: 12.83 },
    cam: 12,
  },
  senora_freeway: { // DONE
    registerzone: {
      center: { x: 2677.05, y: 3279.99, z: 55.24 },
      width: 4.5,
      length: 1.5,
      options: {
        heading: 150.87,
        minZ: 53.24,
        maxZ: 57.24,
      },
    },
    storezone: {
      center: { x: 2676.18, y: 3285.83, z: 55.24 },
      width: 12,
      length: 13,
      options: {
        heading: 60.87,
        minZ: 53.24,
        maxZ: 57.24,
      },
    },
    safecoords: { x: 2674.35, y: 3289.16, z: 55.24 },
    cam: 13,
  },
  strawberry: { // DONE
    registerzone: {
      center: { x: 24.5, y: -1346.1, z: 29.5 },
      width: 4.5,
      length: 1.5,
      options: {
        heading: 90,
        minZ: 27.5,
        maxZ: 31.5,
      },
    },
    storezone: {
      center: { x: 29.2, y: -1342.56, z: 29.5 },
      width: 12,
      length: 13,
      options: {
        heading: 0,
        minZ: 27.5,
        maxZ: 31.5,
      },
    },
    safecoords: { x: 31.24, y: -1339.32, z: 29.5 },
    cam: 14,
  },
  harmony: { // DONE
    registerzone: {
      center: { x: 549.19, y: 2670.15, z: 42.16 },
      width: 4.5,
      length: 1.5,
      options: {
        heading: -82.49,
        minZ: 40.16,
        maxZ: 44.16,
      },
    },
    storezone: {
      center: { x: 545.0, y: 2666.02, z: 42.16 },
      width: 12,
      length: 13,
      options: {
        heading: -172.49,
        minZ: 40.16,
        maxZ: 44.16,
      },
    },
    safecoords: { x: 543.33, y: 2662.52, z: 42.15 },
    cam: 15,
  },
  vinewood: { // DONE
    registerzone: {
      center: { x: 372.88, y: 327.55, z: 103.57 },
      width: 4.5,
      length: 1.5,
      options: {
        heading: -104.11,
        minZ: 101.57,
        maxZ: 105.57,
      },
    },
    storezone: {
      center: { x: 378.25, y: 329.88, z: 103.57 },
      width: 12,
      length: 13,
      options: {
        heading: -14.11,
        minZ: 101.57,
        maxZ: 105.57,
      },
    },
    safecoords: { x: 381.11, y: 332.55, z: 103.57 },
    cam: 16,
  },
  mount_chiliad: { // DONE
    registerzone: {
      center: { x: 1728.38, y: 6416.24, z: 35.04 },
      width: 4.5,
      length: 1.5,
      options: {
        heading: -116.35,
        minZ: 33.04,
        maxZ: 37.04,
      },
    },
    storezone: {
      center: { x: 1734.17, y: 6417.37, z: 35.04 },
      width: 12,
      length: 13,
      options: {
        heading: -26.35,
        minZ: 33.04,
        maxZ: 37.04,
      },
    },
    safecoords: { x: 1737.44, y: 6419.43, z: 35.04 },
    cam: 17,
  },
  sandy_shores: { // DONE
    registerzone: {
      center: { x: 1959.55, y: 3741.01, z: 32.34 },
      width: 4.5,
      length: 1.5,
      options: {
        heading: 119.99,
        minZ: 30.34,
        maxZ: 34.34,
      },
    },
    storezone: {
      center: { x: 1961.79, y: 3746.45, z: 32.34 },
      width: 12,
      length: 13,
      options: {
        heading: 29.99,
        minZ: 30.34,
        maxZ: 34.34,
      },
    },
    safecoords: { x: 1962.13, y: 3750.43, z: 32.34 },
    cam: 18,
  },
  tataviam_mountains: { // DONE
    registerzone: {
      center: { x: 2556.02, y: 380.9, z: 108.62 },
      width: 4.5,
      length: 1.5,
      options: {
        heading: 177.72,
        minZ: 106.62,
        maxZ: 110.62,
      },
    },
    storezone: {
      center: { x: 2552.75, y: 385.68, z: 108.62 },
      width: 12,
      length: 13,
      options: {
        heading: 87.72,
        minZ: 106.62,
        maxZ: 110.62,
      },
    },
    safecoords: { x: 2549.47, y: 387.95, z: 108.62 },
    cam: 19,
  },
  del_vecchio: { // DONE
    registerzone: {
      center: { x: -161.07, y: 6321.37, z: 31.59 },
      width: 1,
      length: 1.5,
      options: {
        heading: 134.99,
        minZ: 29.59,
        maxZ: 33.59,
      },
    },
    storezone: {
      center: { x: -161.93, y: 6320.62, z: 31.59 },
      width: 9,
      length: 16,
      options: {
        heading: 134.99,
        minZ: 29.59,
        maxZ: 33.59,
      },
    },
    safecoords: { x: -168.46, y: 6318.8, z: 30.59 },
    cam: 20,
  },
  paleto_bay: { // DONE
    registerzone: {
      center: { x: 161.34, y: 6642.4, z: 31.7 },
      width: 4.5,
      length: 1.5,
      options: {
        heading: -134.99,
        minZ: 29.7,
        maxZ: 33.7,
      },
    },
    storezone: {
      center: { x: 167.17, y: 6641.61, z: 31.7 },
      width: 12,
      length: 13,
      options: {
        heading: -44.99,
        minZ: 29.7,
        maxZ: 33.7,
      },
    },
    safecoords: { x: 170.93, y: 6642.52, z: 31.7 },
    cam: 21,
  },
};
