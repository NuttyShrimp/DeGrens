...
RegisterServerEvent("dg-cornerselling:server:SellDrugs", function(sellLocation)
  ...
  if DGX.Inventory.removeItemFromPlayer(src, itemData.name) then
    ...
    DGX.Util.Log('cornerselling:sale', {
      ...
      string.format("%s has made sale of %s via cornerselling", GetPlayerName(src),
        DGX.Inventory.getItemData(itemData.name).label), src)
    ...
    local callRNG = math.random(100)
    if callRNG < tonumber(DGX.Config.getConfigValue('dispatch.callChance.cornersell')) then
      DGX.Police.createDispatchCall({
        tag = "10-31",
        title = 'Verdachte situatie',
        description = "Meerdere personen verichten verdachte activiteiten",
        coords = DGX.Util.ArrayToVector3(exports['dg-lib']:vectorToTable(sellLocation)),
        criminal = src,
        blip = {
          sprite = 102,
          color = 0
