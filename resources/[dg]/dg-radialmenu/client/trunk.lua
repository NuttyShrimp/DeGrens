...
AddEventHandler('qb-trunk:client:KidnapTrunk', function()
    ...
    if (distance ~= -1 and distance < 2) then
        if isKidnapping then
            ...
            DGCore.Functions.Notify('You did not kipnap this person!', 'error')
        end
    end
end)
...
AddEventHandler('qb-trunk:client:KidnapGetIn', function(veh)
    ...
    if Config.TrunkClasses[vehClass].allowed then
        DGCore.Functions.TriggerCallback('qb-trunk:server:getTrunkBusy', function(isBusy)
                    ...
                    if not isBusy then
                        if not isKidnapped then
                            -- if GetVehicleDoorAngleRatio(closestVehicle, 5) > 0 then
                                ...
                                DGCore.Functions.Notify('You\'re in the trunk.', 'success', 4000)
                                TrunkCam(true)

                                isKidnapped = true
                            -- else
                                -- DGCore.Functions.Notify('Is the trunk closed?', 'error', 2500)
                            -- end
                        else
                            local ped = PlayerPedId()
                            local vehicle = GetEntityAttachedTo(PlayerPedId())
                            ...
                            if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                                ...
                                DGCore.Functions.Notify('The trunk is closed?', 'error', 2500)
                            end
                        end
                    else
                        DGCore.Functions.Notify('Anyone in there yet?', 'error', 2500)
                    end 
                else
                    DGCore.Functions.Notify('Your already in the trunk', 'error', 2500)
                end 
            else
                DGCore.Functions.Notify('You cant get in this trunk..', 'error', 2500)
            end
        end, plate)
    else
        DGCore.Functions.Notify('You cant get in this trunk..', 'error', 2500)
...
AddEventHandler('qb-trunk:client:GetIn', function(isKidnapped)
    ...
    if closestVehicle ~= 0 then
                    ...
                    if not inTrunk then
                        if not isBusy then
                            if GetVehicleDoorAngleRatio(closestVehicle, 5) > 0 then
                                ...
                                Citizen.Wait(500)
                                SetVehicleDoorShut(closestVehicle, 5, false)
                                DGCore.Functions.Notify('You are already in the trunk.', 'Goodluck', 4000)
                                TrunkCam(true)
                            else
                                DGCore.Functions.Notify('Is the trunk closed?', 'error', 2500)
                            end
                        else
                            DGCore.Functions.Notify('Looks like there is some 1 in there?', 'error', 2500)
                        end 
                    else
                        DGCore.Functions.Notify('You are already in the trunk', 'error', 2500)
                    end 
                else
                    DGCore.Functions.Notify('You cant get in this trunk..', 'error', 2500)
                end
        ...
        DGCore.Functions.Notify('There is no vehicle to see.', 'error', 2500)
    end
end)
