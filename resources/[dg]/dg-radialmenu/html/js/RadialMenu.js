...
RadialMenu.prototype.createCenter = function (svg, title, icon, size) {
  ...
  if (icon) {
    var use = self.createUseTag(0, 0, icon, g, true);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
RadialMenu.prototype.createUseTag = function (x, y, link, parent, center) {
  let iconInfo = window.FontAwesome.parse.icon(link);
  if (!iconInfo) {
    console.error(`Icon "${link}" could not be found, replacing a question mark.`);
    iconInfo = window.FontAwesome.parse.icon('question-circle');
  }
  const faInfo = window.FontAwesome.findIconDefinition(iconInfo).icon;
  ...
  svg.setAttribute('viewBox', `0 0 ${faInfo[0]} ${faInfo[1]}`);
...
RadialMenu.prototype.appendSectorPath = function (startAngleDeg, endAngleDeg, svg, item, index) {
  ...
  if (item) {
    ...
    if (item.icon) {
      var use = self.createUseTag(centerPoint.x, centerPoint.y, item.icon, g);
      if (item.title) {
...
const getIconInfo = icon => {
  ...
  if (iconLibs.includes(iconSplitted[0])) {
    window.___FONT_AWESOME___.styles[iconSplitted[0]]?.[iconInfo.name];
    return {
      lib: iconSplitted[0],
      name: icon.replace(iconSplitted[0] + ' ', ''),
    };
  }
  return {
    lib: 'fa',
