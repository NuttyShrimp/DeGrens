...
AddEventHandler('jobs:client:ToggleNpc', function()
    if DGCore.Functions.GetPlayerData().job.name == "tow" then
        if CurrentTow ~= nil then 
            DGCore.Functions.Notify("First Finish Your Work", "error")
            return
        end
        NpcOn = not NpcOn
        if NpcOn then
...
AddEventHandler('qb-tow:client:TowVehicle', function()
    ...
    if isTowVehicle(vehicle) then
        if CurrentTow == nil then 
            --[[ Replaced "DGCore.Functions.GetClosestVehicle()" with custom implementation "getVehicleInDirection"
                 DGCore native could not return polcice and other vehicles types (NPC) ]] 
            ...
            if NpcOn and CurrentLocation ~= nil then
                if GetEntityModel(targetVehicle) ~= GetHashKey(CurrentLocation.model) then
                    DGCore.Functions.Notify("This Is Not The Right Vehicle", "error")
                    return
                end
            end
            if not IsPedInAnyVehicle(PlayerPedId()) then
                if vehicle ~= targetVehicle then
                    ...
                    if #(towPos - targetPos) < 11.0 then
                        DGCore.Functions.Progressbar("towing_vehicle", "Hoisting the Vehicle...", 5000, false, true, {
                            ...
                            if NpcOn then
                                RemoveBlip(CurrentBlip)
                                DGCore.Functions.Notify("Take The Vehicle To Hayes Depot", "success", 5000)
                            ...
                            DGCore.Functions.Notify("Vehicle Towed")
                        end, function() -- Cancel
                            StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_ped", 1.0)
            ...
            DGCore.Functions.Progressbar("untowing_vehicle", "Remove The Vehicle", 5000, false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
                ...
                CurrentTow = nil
                DGCore.Functions.Notify("Vehicle Taken Off")
            end, function() -- Cancel
                StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_ped", 1.0)
                DGCore.Functions.Notify("Failed", "error")
            end)
        ...
        DGCore.Functions.Notify("You Must Have Been In A Towing Vehicle First", "error")
    end
end)
            ...
            if PlayerJob.name == "tow" then
                    ...
                    if #(pos - vector3(Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z)) < 1.5 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            if JobsDone > 0 then
                                ...
                                DGCore.Functions.Notify("You Havent Done Any Work Yet", "error")
                            end
                        end
                ...
                if NpcOn and CurrentLocation ~= nil and next(CurrentLocation) ~= nil then
                    if #(pos - vector3(CurrentLocation.x, CurrentLocation.y, CurrentLocation.z)) < 50.0 and not VehicleSpawned then
                        VehicleSpawned = true
                        DGCore.Functions.SpawnVehicle(CurrentLocation.model, function(veh)
...
function deliverVehicle(vehicle)
    ...
    JobsDone = JobsDone + 1
    VehicleSpawned = false
    DGCore.Functions.Notify("You Have Delivered A Vehicle", "success")
    DGCore.Functions.Notify("A New Vehicle Can Be Picked Up")
...
AddEventHandler('qb-tow:client:SpawnVehicle', function()
    ...
    DGCore.Functions.SpawnVehicle(vehicleInfo, function(veh)
