        ...
        for k, v in pairs(Player.PlayerData.items) do 
            if Player.PlayerData.items[k] ~= nil then 
                if MeltItems[Player.PlayerData.items[k].name] ~= nil then 
                    ...
                    if amount < 1 then
                        TriggerClientEvent('DGCore:Notify', src, "You do not have enough " .. Player.PlayerData.items[k].label, "error")
                    else
...
DGCore.Functions.CreateCallback('qb-pawnshop:server:getSellPrice', function(source, cb)
    local retval = 0
...
DGCore.Functions.CreateCallback('qb-pawnshop:melting:server:GetConfig', function(source, cb)
    cb(Config.IsMelting, Config.MeltTime, Config.CanTake)
end)

DGCore.Functions.CreateCallback('qb-pawnshop:server:getSellHardwarePrice', function(source, cb)
    local retval = 0
    local Player = DGCore.Functions.GetPlayer(source)
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
        for k, v in pairs(Player.PlayerData.items) do 
...
DGCore.Functions.CreateCallback('qb-pawnshop:server:hasGold', function(source, cb)
	local retval = false
    local Player = DGCore.Functions.GetPlayer(source)
