				...
				if GetClockHours() >= 7 and GetClockHours() <= 17 then
					...
					elseif sellItemsSet and sellPrice ~= 0 then
						...
						if IsControlJustReleased(0, 38) then
                            ...
                            DGCore.Functions.Progressbar("sell_pawn_items", "Selling Items", math.random(15000, 25000), false, true, {}, {}, {}, {}, function() -- Done
			...
			if #(pos - Config.PawnHardwareLocation) < 1.5 then
				if GetClockHours() >= 9 and GetClockHours() <= 16 then
					...
					elseif sellHardwareItemsSet and sellHardwarePrice ~= 0 then
						...
						if IsControlJustReleased(0, 38) then
							TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_STAND_IMPATIENT", 0, true)
                            DGCore.Functions.Progressbar("sell_pawn_items", "Sell things", math.random(15000, 25000), false, true, {}, {}, {}, {}, function() -- Done
                                ClearPedTasks(PlayerPedId())
...
function GetSellingPrice()
	local price = 0
	DGCore.Functions.TriggerCallback('qb-pawnshop:server:getSellPrice', function(result)
		price = result
	end)
	Citizen.Wait(500)
	return price
...
function GetSellingHardwarePrice()
	local price = 0
	DGCore.Functions.TriggerCallback('qb-pawnshop:server:getSellHardwarePrice', function(result)
		price = result
	end)
	Citizen.Wait(500)
	return price
