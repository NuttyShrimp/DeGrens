...
AddEventHandler("DGCore:Client:OnPlayerLoaded", function()
    DGCore.Functions.TriggerCallback('qb-diving:server:GetBusyDocks', function(Docks)
...
AddEventHandler('qb-diving:client:UseJerrycan', function()
    ...
    if boat then
        ...
        DGCore.Functions.Progressbar("reful_boat", "Refueling boat ..", 20000, false, true, {
            ...
            TriggerEvent('inventory:client:ItemBox', exports["dg-inventory"]:GetItemData()['jerry_can'], "remove")
        end, function() -- Cancel
            DGCore.Functions.Notify('Refueling has been canceled!', 'error')
        end)
    else
        DGCore.Functions.Notify('You are not in a boat', 'error')
    end
end)
