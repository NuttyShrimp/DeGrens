...
AddEventHandler('qb-streetraces:RaceDone', function(race, winner)
    if RaceId ~= 0 and RaceId == race then
        RaceId = 0
        InRace = false
        DGCore.Functions.Notify("Race Is Over! The Winner Is "..winner.. "!")
    end
end)
...
AddEventHandler('qb-streetraces:CreateRace', function(amount)
    ...
    if DoesBlipExist(WaypointHandle) then
        ...
        if #(pos - vector3(c[1], c[2], groundZ)) > 500.0 then
            ...
            DGCore.Functions.Notify("Race Made For $"..amount.."", "success")
        else
            DGCore.Functions.Notify("End Position Is Too Close", "error")
        end
    else
        DGCore.Functions.Notify("You Need To Drop A Marker", "error")
    end
end)
...
function RaceCountDown()
    ...
    while RaceCount ~= 0 do
        ...
        DGCore.Functions.Notify(RaceCount, 'primary', 800)
        Citizen.Wait(1000)
    ...
    DGCore.Functions.Notify("GOOOOOOOOO!!!")
end
