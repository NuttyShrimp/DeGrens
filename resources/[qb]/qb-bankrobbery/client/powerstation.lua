        ...
        if dist < 1.5 then
            if CurrentCops >= Config.MinimumThermitePolice then
                if not Config.PowerStations[closestStation].hit then
                    ...
                    DGCore.Functions.Notify("It seems that the fuses have blown.", "error")
                end
            else
                DGCore.Functions.Notify('Minimum Of '..Config.MinimumThermitePolice..' Police Needed', "error")
            end
        end
...
RegisterNUICallback('thermitesuccess', function()
    ...
    local coords = GetEntityCoords(PlayerPedId())
    while time > 0 do 
        DGCore.Functions.Notify("Thermite is going off in " .. time .. "..")
        Citizen.Wait(1000)
        time = time - 1
    end
    local randTime = math.random(10000, 15000)
    ...
    elseif currentGate ~= 0 then
        DGCore.Functions.Notify("The door is open", "success")
