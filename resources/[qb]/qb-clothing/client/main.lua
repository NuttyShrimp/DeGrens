...
RegisterNUICallback('TrackerError', function()
    DGCore.Functions.Notify("You can't remove your ankle bracelet ..", "error")
end)
...
RegisterNUICallback('removeOutfit', function(data, cb)
    ...
    DGCore.Functions.Notify("You have deleted your"..data.outfitName.." outfit!")
end)
...
AddEventHandler('qb-clothing:client:loadOutfit', function(oData)
    ...
    if data["accessory"] ~= nil then
        if DGCore.Functions.GetPlayerData().metadata["tracker"] then
    ...
    if oData.outfitName ~= nil then
        DGCore.Functions.Notify("You have chosen "..oData.outfitName.."! Press Confirm to confirm outfit.")
    end
end)
