    ...
    while true do
		...
		if MissionStart == 1 then
			...
			if dist <= 55.0  then
				...
				if warning == 0 then
				warning = 1
				DGCore.Functions.Notify("Get rid of the guards before you place the bomb.", "error")
				end
...
if IsVehicleStopped(transport) then
	if IsVehicleSeatFree(transport, -1) and IsVehicleSeatFree(transport, 0) and IsVehicleSeatFree(transport, 1) and GuardsDead == 1 then
		if not IsEntityInWater(PlayerPedId()) then
			...
			DGCore.Functions.Notify('The load will be detonated in '..TimeToBlow / 1000 ..' seconds.', "error")
			...
			BlownUp = 1
			lootable = 1
			DGCore.Functions.Notify('You can start collecting cash.', "success")
			RemoveBlip(TruckBlip)
		else
			DGCore.Functions.Notify('Get out of the water', "error")
		end
	else
		DGCore.Functions.Notify('The vehicle must be empty to place the load', "error")
	end
else
	DGCore.Functions.Notify('You cant rob a vehicle that is moving.', "error")
end
end
...
function TakingMoney()
	...
	FreezeEntityPosition(PlayerPedId(), true)
	DGCore.Functions.Notify('You are packing cash into a bag', "success")
