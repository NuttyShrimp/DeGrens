...
AddEventHandler('qb-weed:client:getHousePlants', function(house)    
    DGCore.Functions.TriggerCallback('qb-weed:server:getBuildingPlants', function(plants)
        currentHouse = house
                ...
                for k, v in pairs(housePlants[currentHouse]) do
                    ...
                    if plyDistance < 0.8 then
                        ...
                        if plantData["plantStats"]["health"] > 0 then
                            if plantData["plantStage"] ~= plantData["plantStats"]["highestStage"] then
                                ...
                                if IsControlJustPressed(0, 38) then
                                    DGCore.Functions.Progressbar("remove_weed_plant", "Harvesting Plant", 8000, false, true, {
                        ...
                        elseif plantData["plantStats"]["health"] == 0 then
                            ...
                            if IsControlJustPressed(0, 38) then
                                DGCore.Functions.Progressbar("remove_weed_plant", "Removing The Plant", 8000, false, true, {
                                    disableMovement = true,
                                    disableCarMovement = true,
                                    disableMouse = false,
                                    disableCombat = true,
...
AddEventHandler('qb-weed:client:placePlant', function(type, item)
    ...
    if currentHouse ~= nil then
        if ClosestPlant == 0 then
            DGCore.Functions.Progressbar("plant_weed_plant", "Planting", 8000, false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
            ...
            DGCore.Functions.Notify("Can't Place Here", 'error', 3500)
        end
    else
        DGCore.Functions.Notify("It's Not Safe Here, try your house", 'error', 3500)
    end
end)
        ...
        if ClosestTarget ~= 0 then
            ...
            if plyDistance < 1.0 then
                if plantData["plantStats"]["food"] == 100 then
                    DGCore.Functions.Notify('The Plant Does Not Need Nutrition', 'error', 3500)
                else
                    DGCore.Functions.Progressbar("plant_weed_plant", "Feeding Plant", math.random(4000, 8000), false, true, {
                        disableMovement = true,
                        disableCarMovement = true,
                        disableMouse = false,
                        disableCombat = true,
                ...
                DGCore.Functions.Notify("Can't Place Here", "error")
            end
        else
            DGCore.Functions.Notify("Can't Place Here", "error")
        end
