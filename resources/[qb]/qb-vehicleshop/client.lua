...
local PlayerData = DGCore.Functions.GetPlayerData() -- Just for resource restart (same as event handler)
...
local function getVehName()
    return DGCore.Shared.Vehicles[Config.Shops[ClosestShop]["ShowroomVehicles"][ClosestVehicle].chosenVehicle]["name"]
end

local function getVehPrice()
    return comma_value(DGCore.Shared.Vehicles[Config.Shops[ClosestShop]["ShowroomVehicles"][ClosestVehicle].chosenVehicle]["price"])
end

local function getVehBrand()
    return DGCore.Shared.Vehicles[Config.Shops[ClosestShop]["ShowroomVehicles"][ClosestVehicle].chosenVehicle]["brand"]
end
...
RegisterNetEvent('qb-vehicleshop:client:TestDrive', function()
    if not inTestDrive and ClosestVehicle ~= 0 then
        ...
        DGCore.Functions.SpawnVehicle(Config.Shops[ClosestShop]["ShowroomVehicles"][ClosestVehicle].chosenVehicle, function(veh)
            ...
            TriggerServerEvent('qb-vehicletuning:server:SaveVehicleProps', DGCore.Functions.GetVehicleProperties(veh))
            testDriveVeh = veh
            DGCore.Functions.Notify('You have '..Config.Shops[ClosestShop]["TestDriveTimeLimit"]..' minutes remaining')
            SetTimeout(Config.Shops[ClosestShop]["TestDriveTimeLimit"] * 60000, function()
                if testDriveVeh ~= 0 then
                    ...
                    DGCore.Functions.Notify('Vehicle test drive complete')
                end
            end)
        ...
        DGCore.Functions.Notify('Already in test drive', 'error')
    end
end)
...
RegisterNetEvent('qb-vehicleshop:client:customTestDrive', function(data)
    if not inTestDrive then
        ...
        local prevCoords = GetEntityCoords(PlayerPedId())
        DGCore.Functions.SpawnVehicle(vehicle, function(veh)
            exports['LegacyFuel']:SetFuel(veh, 100)
            SetVehicleNumberPlateText(veh, 'TESTDRIVE')
            SetEntityAsMissionEntity(veh, true, true)
            SetEntityHeading(veh, Config.Shops[ClosestShop]["VehicleSpawn"].w)
...
RegisterNetEvent('qb-vehicleshop:client:swapVehicle', function(data)
    if Config.Shops[data.ClosestShop]["ShowroomVehicles"][data.ClosestVehicle].chosenVehicle ~= data.toVehicle then
        DGCore.Functions.DeleteVehicle(GetClosestVehicle(Config.Shops[data.ClosestShop]["ShowroomVehicles"][data.ClosestVehicle].coords.x, Config.Shops[data.ClosestShop]["ShowroomVehicles"][data.ClosestVehicle].coords.y, Config.Shops[data.ClosestShop]["ShowroomVehicles"][data.ClosestVehicle].coords.z, 3.0, 0, 70))
        Wait(250)
...
RegisterNetEvent('qb-vehicleshop:client:getVehicles', function()
    DGCore.Functions.TriggerCallback('qb-vehicleshop:server:getVehicles', function(vehicles)
        local ownedVehicles = {}
