                    ...
                    if distance < 1.5 then
                        if InVehicle then
                            ...
                            if IsControlJustReleased(0, 38) then
                                DGCore.Functions.TriggerCallback('qb-garbagejob:server:CheckBail', function(DidBail)
                                    if DidBail then
                                        BringBackCar()
                                        DGCore.Functions.Notify("You have $250,- deposit returned!")
                                    else
                                        DGCore.Functions.Notify("You have no deposit paid on this vehicle..")
                                    end
                                end)
                            end
                        else
                            ...
                            if IsControlJustReleased(0, 38) then
                                DGCore.Functions.TriggerCallback('qb-garbagejob:server:HasMoney', function(HasMoney)
                                    if HasMoney then
                                        local coords = Config.Locations["vehicle"].coords
                                        DGCore.Functions.SpawnVehicle("trash2", function(veh)
                                            GarbageVehicle = veh
                                            ...
                                            IsWorking = true
                                            SetGarbageRoute()
                                            DGCore.Functions.Notify("You have $250,- deposit paid!")
                                            DGCore.Functions.Notify("You have started working, location marked on GPS!")
                                        end, coords, true)
                                    else
                                        DGCore.Functions.Notify("You have not enough money for the deposit.. Deposit costs are $1000,-")
                                    end
                                end)
                            end
                        end
                                            ...
                                            if IsControlJustPressed(0, 51) then
                                                ...
                                                if (AmountOfBags - 1) == 0 then
                                                    ...
                                                    if (GarbageLocation + 1) <= AmountOfLocations then
                                                        ...
                                                        SetGarbageRoute()
                                                        DGCore.Functions.Notify("All garbage bags are done, proceed to the next location!")
                                                    else
                                                        -- Finished work
                                                        DGCore.Functions.Notify("You are done working! Go back to the depot.")
                                                        IsWorking = false
                                                        RemoveBlip(DeliveryBlip)
                                                        SetRouteBack()
                                                    end
                                                    ...
                                                    if AmountOfBags > 1 then
                                                        DGCore.Functions.Notify("There are still "..AmountOfBags.." bags left!")
                                                    else
                                                        DGCore.Functions.Notify("There is still "..AmountOfBags.." bags over there!")
                                                    end
                                                    hasGarbageBag = false
                                                end
                                                DeliverAnim()
