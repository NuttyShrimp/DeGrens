...
AddEventHandler('qb-banking:server:unpackMoneyBag', function(item)
    local _src = source
    if item ~= nil then
        local xPlayer = DGCore.Functions.GetPlayer(_src)
...
AddEventHandler('qb-banking:initiateTransfer', function(data)
    --[[
    local _src = source
    local _startChar = DGCore.Functions.GetPlayer(_src)
...
DGCore.Functions.CreateCallback('qb-banking:getBankingInformation', function(source, cb)
    local src = source
    local xPlayer = DGCore.Functions.GetPlayer(src)
...
AddEventHandler('qb-banking:createBankCard', function(pin)
    ...
    TriggerClientEvent('DGCore:Notify', src, 'You have successfully ordered a Debit Card.', 'success')
...
DGCore.Commands.Add('givecash', 'Give cash to player.', {{name = 'id', help = 'Player ID'}, {name = 'amount', help = 'Amount'}}, true, function(source, args)
    local src = source
	...
	if id and amount then
		local xPlayer = DGCore.Functions.GetPlayer(src)
		local xReciv = DGCore.Functions.GetPlayer(id)
				...
				if #(GetEntityCoords(GetPlayerPed(src)) - GetEntityCoords(GetPlayerPed(id))) < distance then
					if xPlayer.Functions.RemoveMoney('cash', amount) then
						if xReciv.Functions.AddMoney('cash', amount) then
							TriggerClientEvent('DGCore:Notify', src, "Success fully gave to ID " .. tostring(id) .. ' ' .. tostring(amount) .. '$.', "success")
							TriggerClientEvent('DGCore:Notify', id, "Success recived gave " .. tostring(amount) .. '$ from ID ' .. tostring(src), "success")
							TriggerClientEvent("payanimation", src)
						else
							TriggerClientEvent('DGCore:Notify', src, "Could not give item to the given id.", "error")
						end
					else
						TriggerClientEvent('DGCore:Notify', src, "You don\'t have this amount.", "error")
					end
				else
					TriggerClientEvent('DGCore:Notify', src, "You are too far away lmfao.", "error")
				end
			else
				TriggerClientEvent('DGCore:Notify', src, "You are dead LOL.", "error")
			end
		else
			TriggerClientEvent('DGCore:Notify', src, "Wrong ID.", "error")
		end
	else
		TriggerClientEvent('DGCore:Notify', src, "Usage /givecash [ID] [AMOUNT]", "error")
	end
end)
