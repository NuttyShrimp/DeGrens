...
RegisterNUICallback('save', function(data)
    DGCore.Functions.TriggerCallback('qb-tunerchip:server:HasChip', function(HasChip)
        if HasChip then
            local ped = PlayerPedId()
            local veh = GetVehiclePedIsUsing(ped)
            setVehData(veh, data)
            DGCore.Functions.Notify('TunerChip v1.05: Vehicle Tuned!', 'error')
...
AddEventHandler('qb-tunerchip:client:TuneStatus', function()
    ...
    if vehModel ~= 0 then
        DGCore.Functions.TriggerCallback('qb-tunerchip:server:GetStatus', function(status)
            if status then
                DGCore.Functions.Notify('This Vehicle Has Been Tuned', 'success')
            else
                DGCore.Functions.Notify('This Vehicle Has Not Been Tuned', 'error')
            end
        end, plate)
    else
        DGCore.Functions.Notify('No Vehicle Nearby', 'error')
    end
end)
...
RegisterNUICallback('reset', function(data)
    ...
    local veh = GetVehiclePedIsUsing(ped)
    resetVeh(veh)
    DGCore.Functions.Notify('TunerChip v1.05: Vehicle has been reset!', 'error')
end)
...
AddEventHandler('qb-tunerchip:client:openChip', function()
    ...
    if inVehicle then
        DGCore.Functions.Progressbar("connect_laptop", "Tunerchip v1.05: Vehicle Has Been Reset!", 2000, false, true, {
