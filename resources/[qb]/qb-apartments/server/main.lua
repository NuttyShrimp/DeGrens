...
RegisterNetEvent('apartments:server:CreateApartment', function(type, label)
    ...
    TriggerClientEvent('DGCore:Notify', src, "You got a apartment ("..label..")")
...
RegisterNetEvent('apartments:server:UpdateApartment', function(type, label)
    ...
    TriggerClientEvent('DGCore:Notify', src, "You have changed apartments")
    TriggerClientEvent("apartments:client:SetHomeBlip", src, type)
end)
...
DGCore.Functions.CreateCallback('apartments:GetAvailableApartments', function(source, cb, apartment)
    local apartments = {}
...
DGCore.Functions.CreateCallback('apartments:GetApartmentOffset', function(source, cb, apartmentId)
    local retval = 0
...
DGCore.Functions.CreateCallback('apartments:GetApartmentOffsetNewOffset', function(source, cb, apartment)
...
DGCore.Functions.CreateCallback('apartments:GetOwnedApartment', function(source, cb, cid)
    if cid ~= nil then
...
DGCore.Functions.CreateCallback('apartments:IsOwner', function(source, cb, apartment)
	local src = source
    local Player = DGCore.Functions.GetPlayer(src)
    if Player ~= nil then
        local result = exports.oxmysql:executeSync('SELECT * FROM apartments WHERE citizenid = ?', { Player.PlayerData.citizenid })
...
DGCore.Functions.CreateCallback('apartments:GetOutfits', function(source, cb)
	local src = source
	local Player = DGCore.Functions.GetPlayer(src)
