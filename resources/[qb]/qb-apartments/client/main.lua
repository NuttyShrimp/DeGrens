...
local function EnterApartment(house, apartmentId, new)
    ...
    openHouseAnim()
    Citizen.Wait(250)
    DGCore.Functions.TriggerCallback('apartments:GetApartmentOffset', function(offset)
        if offset == nil or offset == 0 then
            DGCore.Functions.TriggerCallback('apartments:GetApartmentOffsetNewOffset', function(newoffset)
...
local function LeaveApartment(house)
    ...
    exports['qb-interior']:DespawnInterior(houseObj, function()
        ...
        DoScreenFadeIn(1000)
        TriggerServerEvent("InteractSound_SV:PlayOnSource", "houses_door_close", 0.1)
        TriggerServerEvent("DGCore:Server:SetMetaData", "currentapartment", nil)
    end)
end
...
local function SetClosestApartment()
    ...
    if current ~= ClosestHouse and LocalPlayer.state['isLoggedIn'] and not InApartment then
        ...
        DGCore.Functions.TriggerCallback('apartments:IsOwner', function(result)
            IsOwned = result
        end, ClosestHouse)
    end
end
...
function OwnerList()
    DGCore.Functions.TriggerCallback('apartments:GetAvailableApartments', function(apartments)
        ped = PlayerPedId();
        MenuTitle = "Rang the door at: "
        ClearMenu()

        if apartments == nil then
            DGCore.Functions.Notify("There is nobody home..", "error", 3500)
            closeMenuFull()
        else
...
RegisterNetEvent('apartments:client:setupSpawnUI', function(cData)
    DGCore.Functions.TriggerCallback('apartments:GetOwnedApartment', function(result)
        if result then
...
RegisterNetEvent('apartments:client:SpawnInApartment', function(apartmentId, apartment)
    ...
    if rangDoorbell ~= nil then
        ...
        if doorbelldist > 5 then
            DGCore.Functions.Notify("You are to far away from the Doorbell")
            return
        end
    end
...
RegisterNetEvent('apartments:client:RingDoor', function(player, house)
    ...
    TriggerServerEvent("InteractSound_SV:PlayOnSource", "doorbell", 0.1)
    DGCore.Functions.Notify("Someone Is At The Door!")
end)
