                            ...
                            if dist < 0.6 then
                                if not Config.Locations[case]["isBusy"] and not Config.Locations[case]["isOpened"] then
                                    ...
                                    if IsControlJustPressed(0, 38) then
                                        DGCore.Functions.TriggerCallback('qb-jewellery:server:getCops', function(cops)
                                            if cops >= Config.RequiredCops then
                                                if validWeapon() then
                                                    smashVitrine(case)
                                                else
                                                    DGCore.Functions.Notify('Your weapon is not strong enough..', 'error')
                                                end
                                            else
                                                DGCore.Functions.Notify('Not Enough Police ('.. Config.RequiredCops ..') Required', 'error')
                                            end                
                                        end)
                                    end
                                end
...
function smashVitrine(k)
    ...
    elseif math.random(1, 100) <= 5 and IsWearingHandshoes() then
        TriggerServerEvent("evidence:server:CreateFingerDrop", plyCoords)
        DGCore.Functions.Notify("You've left a fingerprint on the glass", "error")
    end

    smashing = true

    DGCore.Functions.Progressbar("smash_vitrine", "Stocking a display", Config.WhitelistedWeapons[pedWeapon]["timeOut"], false, true, {
