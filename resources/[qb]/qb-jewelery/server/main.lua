...
AddEventHandler('qb-jewellery:server:vitrineReward', function()
    ...
    if otherchance == odd then
        ...
        if Player.Functions.AddItem(Config.VitrineRewards[item]["item"], amount) then
            TriggerClientEvent('inventory:client:ItemBox', src, DGCore.Shared.Items[Config.VitrineRewards[item]["item"]], 'add')
        else
        ...
        if Player.Functions.AddItem("10kgoldchain", amount) then
            TriggerClientEvent('inventory:client:ItemBox', src, DGCore.Shared.Items["10kgoldchain"], 'add')
        else
            TriggerClientEvent('DGCore:Notify', src, 'You have to much in your pocket..', 'error')
        end
    end
