...
local function RefreshCrypto()
    ...
    if result ~= nil and result[1] ~= nil then
        ...
        if result[1].history ~= nil then
            ...
            TriggerClientEvent("DGCore:Notify", -1,"Crypto now stands at "..result[1].worth.."", "success")
...
DGCore.Commands.Add("setcryptoworth", "Set crypto value", {{name="crypto", help="Name of the crypto currency"}, {name="Value", help="New value of the crypto currency"}}, false, function(source, args)
    local src = source
    ...
    if crypto ~= nil then
        if Crypto.Worth[crypto] ~= nil then
            ...
            if NewWorth ~= nil then
                ...
                TriggerClientEvent('DGCore:Notify', src, "You have the value of "..Crypto.Labels[crypto].."adapted from: ($"..Crypto.Worth[crypto].." to: $"..NewWorth..") ("..ChangeLabel.." "..PercentageChange.."%)")
                ...
                TriggerClientEvent('DGCore:Notify', src, "You have not given a new value .. Current values: "..Crypto.Worth[crypto])
            end
        else
            TriggerClientEvent('DGCore:Notify', src, "This Crypto does not exist :(, available: Qbit")
        end
    else
        TriggerClientEvent('DGCore:Notify', src, "You have not provided Crypto, available: Qbit")
    end
end, "admin")

DGCore.Commands.Add("checkcryptoworth", "", {}, false, function(source)
    local src = source
    TriggerClientEvent('DGCore:Notify', src, "The Qbit has a value of: $"..Crypto.Worth["qbit"])
end)

DGCore.Commands.Add("crypto", "", {}, false, function(source)
    local src = source
    local Player = DGCore.Functions.GetPlayer(src)
    local MyPocket = math.ceil(Player.PlayerData.money.crypto * Crypto.Worth["qbit"])

    TriggerClientEvent('DGCore:Notify', src, "You have: "..Player.PlayerData.money.crypto.." QBit, with a value of: $"..MyPocket..",-")
end)
...
RegisterServerEvent('qb-crypto:server:ExchangeFail', function()
    ...
    if ItemData ~= nil then
        ...
        TriggerClientEvent('inventory:client:ItemBox', src, DGCore.Shared.Items["cryptostick"], "remove")
...
DGCore.Functions.CreateCallback('qb-crypto:server:HasSticky', function(source, cb)
...
DGCore.Functions.CreateCallback('qb-crypto:server:GetCryptoData', function(source, cb, name)
    local Player = DGCore.Functions.GetPlayer(source)
    local CryptoData = {
...
DGCore.Functions.CreateCallback('qb-crypto:server:BuyCrypto', function(source, cb, data)
    local Player = DGCore.Functions.GetPlayer(source)
...
DGCore.Functions.CreateCallback('qb-crypto:server:SellCrypto', function(source, cb, data)
    local Player = DGCore.Functions.GetPlayer(source)
...
DGCore.Functions.CreateCallback('qb-crypto:server:TransferCrypto', function(source, cb, data)
    local Player = DGCore.Functions.GetPlayer(source)
    ...
    if Player.PlayerData.money.crypto >= tonumber(data.Coins) then
        ...
        if result[1] ~= nil then
            ...
            local Target = DGCore.Functions.GetPlayerByCitizenId(result[1].citizenid)
