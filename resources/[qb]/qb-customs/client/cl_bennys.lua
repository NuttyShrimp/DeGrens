...
local function saveVehicle()
    ...
    local myCar = DGCore.Functions.GetVehicleProperties(veh)
        ...
        if IsPedInAnyVehicle(plyPed, false) then
            ...
            for k, v in pairs(bennyGarages) do
				...
				if GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId()), -1) == PlayerPedId() then
					if (v.useJob and isAuthorized((DGCore.Functions.GetPlayerData().job.name), k)) or not v.useJob then
					    TriggerEvent('event:control:bennys', k)
					else
					    DGCore.Functions.Notify("You are not authorized", "error")
					end
				end
                            end
                        else
...
RegisterNetEvent("qb-customs:purchaseSuccessful", function()
    ...
    DGCore.Functions.Notify("Purchase Successful")
end)

RegisterNetEvent("qb-customs:purchaseFailed", function()
    isPurchaseSuccessful = false
    attemptingPurchase = false
    DGCore.Functions.Notify("Not enough money", "error")
end)
