            ...
            for _,slot in pairs(Config.OccasionSlots) do
                ...
                if dist <= 40 then
                    ...
                    if not vehiclesSpawned then
                        ...
                        DGCore.Functions.TriggerCallback('qb-occasions:server:getVehicles', function(vehicles)
                    ...
                    if IsControlJustPressed(0, 38) then
                        DGCore.Functions.TriggerCallback('qb-garage:server:checkVehicleOwner', function(owned)
                            if owned then
                                ...
                                DGCore.Functions.Notify('This is not your vehicle..', 'error', 3500)
                            end
                    ...
                    if dstCheck <= 2 then
                        if not IsPedInAnyVehicle(ped) then
                            if not isConfirming then
                                ...
                                DrawText3Ds(vehPos.x, vehPos.y, vehPos.z + 1.25, DGCore.Shared.Vehicles[Config.OccasionSlots[i]["model"]]["name"]..', Price: ~g~$'..Config.OccasionSlots[i]["price"])
                                if Config.OccasionSlots[i]["owner"] == DGCore.Functions.GetPlayerData().citizenid then
                                ...
                                if IsControlJustPressed(0, 38) then
                                    ...
                                    DGCore.Functions.TriggerCallback('qb-occasions:server:getSellerInformation', function(info)
                                        if info ~= nil then 
...
function spawnOccasionsVehicles(vehicles)
    ...
    if vehicles ~= nil then
        for i = 1, #vehicles, 1 do
            ...
            DGCore.Functions.SetVehicleProperties(oSlot[i]["occasionid"], json.decode(oSlot[i]["mods"]))
...
function despawnOccasionsVehicles()
    ...
    for i = 1, #Config.OccasionSlots, 1 do
        ...
        if oldVehicle ~= 0 then
            DGCore.Functions.DeleteVehicle(oldVehicle)
        end
    end
end
...
function openSellContract(bool)
    ...
    SendNUIMessage({
        action = "sellVehicle",
        pData = DGCore.Functions.GetPlayerData(),
...
RegisterNUICallback('error', function(data)
    DGCore.Functions.Notify(data.message, 'error')
end)
...
AddEventHandler('qb-occasions:client:ReturnOwnedVehicle', function(vehdata)
    ...
    DGCore.Functions.SpawnVehicle(vehdata.model, function(veh)
        ...
        TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
        SetVehicleFuelLevel(veh, 100)
        DGCore.Functions.Notify("You vehicle is returned")
        TriggerEvent("vehiclekeys:client:SetOwner", vehdata.plate)
        SetVehicleEngineOn(veh, true, true)
        Citizen.Wait(500)
        DGCore.Functions.SetVehicleProperties(veh, vehmods)
...
function SellData(data,model)
    DGCore.Functions.TriggerCallback("qb-vehiclesales:server:CheckModelName",function(DataReturning) 
        local vehicleData = {}
        ...
        vehicleData.mods = DGCore.Functions.GetVehicleProperties(vehicleData.ent)
...
function sellVehicleWait(price)
    DoScreenFadeOut(250)
    Citizen.Wait(250)
    DGCore.Functions.DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
    Citizen.Wait(1500)
    DoScreenFadeIn(250)
    DGCore.Functions.Notify('Your car has been put up for sale! Price - $'..price, 'success')
