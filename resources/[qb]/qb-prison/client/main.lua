...
AddEventHandler('DGCore:Client:OnPlayerLoaded', function()
	...
	DGCore.Functions.TriggerCallback('prison:server:IsAlarmActive', function(active)
		if active then
	...
	while true do 
		...
		if jailTime > 0 and inJail then 
			...
			if jailTime > 0 and inJail then
				...
				if jailTime <= 0 then
					jailTime = 0
					DGCore.Functions.Notify("Your time is up! Check yourself out at the visitors center", "success", 10000)
				end
...
AddEventHandler('prison:client:Enter', function(time)
	...
	DoScreenFadeIn(1000)
	DGCore.Functions.Notify("Do some work for sentence reduction, instant job: "..Config.Jobs[currentJob])
end)

RegisterNetEvent('prison:client:Leave')
AddEventHandler('prison:client:Leave', function()
	if jailTime > 0 then 
		DGCore.Functions.Notify("You still have to... "..jailTime.." months..")
	else
		jailTime = 0
		...
		RemoveBlip(ShopBlip)
		ShopBlip = nil
		DGCore.Functions.Notify("You're free! Enjoy it! :)")
		DoScreenFadeOut(500)
		while not IsScreenFadedOut() do
			Citizen.Wait(10)
		end
