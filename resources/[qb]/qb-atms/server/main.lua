...
Citizen.CreateThread(function()
    while true do
        Wait(3600000)
        dailyWithdraws = {}
        TriggerClientEvent('DGCore:Notify', -1, "Daily Withdraw Limit Reset", "success")
    end
end)
...
RegisterCommand('atm', function(source)
    ...
    if visas ~= nil and masters ~= nil then
        for _, v in pairs(visas) do
            ...
            local xCH = DGCore.Functions.GetPlayerByCitizenId(cardHolder)
            if xCH ~= nil then
        ...
        if dailyWith < Config.DailyLimit then
            ...
            if xCH ~= nil then
                ...
                if bankCount > 0 then
                    ...
                    TriggerClientEvent('DGCore:Notify', src, "Withdraw $" .. data.amount .. ' from credit card. Daily Withdraws: ' .. dailyWithdraws[cardHolder], "success")
                else
            ...
            TriggerClientEvent('DGCore:Notify', src, "You have reached the daily limit", "error")
        end
    end
end)
...
DGCore.Functions.CreateCallback('qb-debitcard:server:requestCards', function(source, cb)
    local src = source
    local xPlayer = DGCore.Functions.GetPlayer(src)
...
DGCore.Functions.CreateCallback('qb-debitcard:server:deleteCard', function(source, cb, data)
...
DGCore.Functions.CreateCallback('qb-atms:server:loadBankAccount', function(source, cb, cid, cardnumber)
    local src = source
    local xPlayer = DGCore.Functions.GetPlayer(src)
