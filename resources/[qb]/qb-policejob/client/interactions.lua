...
local function IsTargetDead(playerId)
    local retval = false
    DGCore.Functions.TriggerCallback('police:server:isPlayerDead', function(result)
        retval = result
    end, playerId)
    Wait(100)
    return retval
...
RegisterNetEvent('police:client:RobPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if IsEntityPlayingAnim(playerPed, "missminuteman_1ig_2", "handsup_base", 3) or IsEntityPlayingAnim(playerPed, "mp_arresting", "idle", 3) or IsTargetDead(playerId) then
            DGCore.Functions.Progressbar("robbing_player", "Robbing person..", math.random(5000, 7000), false, true, {
    ...
    if not IsPedRagdoll(PlayerPedId()) then
        ...
        if player ~= -1 and distance < 1.5 then
            ...
            if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(GetPlayerPed(PlayerPedId())) then
                ...
                DGCore.Functions.Notify("You cant cuff someone in a vehicle", "error")
            end
        else
            DGCore.Functions.Notify("No one nearby!", "error")
        end
...
RegisterNetEvent('police:client:GetCuffed', function(playerId, isSoftcuff)
    ...
    if not isHandcuffed then
        ...
        if not isSoftcuff then
            ...
            DGCore.Functions.Notify("You are cuffed!")
        else
            cuffType = 49
            GetCuffedAnimation(playerId)
            DGCore.Functions.Notify("You are cuffed, but you can walk")
        end
    else
        isHandcuffed = false
        isEscorted = false
        ...
        ClearPedTasksImmediately(ped)
        DGCore.Functions.Notify("You are uncuffed!")
    end
end)
...
CreateThread(function()
    while true do
        ...
        if isHandcuffed then
            ...
            if (not IsEntityPlayingAnim(PlayerPedId(), "mp_arresting", "idle", 3) and not IsEntityPlayingAnim(PlayerPedId(), "mp_arrest_paired", "crook_p2_back_right", 3)) and not DGCore.Functions.GetPlayerData().metadata["isdead"] then
