...
local function SetClosestHouse()
    ...
    if not inside then
        ...
        if closesthouse ~= nil and tonumber(dist) < 30 then 
            DGCore.Functions.TriggerCallback('qb-houses:server:ProximityKO', function(key, owned)
                hasKey = key
                isOwned = owned
            end, closesthouse)
        end
...
local function setHouseLocations()
    if closesthouse ~= nil then
        DGCore.Functions.TriggerCallback('qb-houses:server:getHouseLocations', function(result)
            if result ~= nil then
...
local function LoadDecorations(house)
	if Config.Houses[house].decorations == nil or next(Config.Houses[house].decorations) == nil then
		DGCore.Functions.TriggerCallback('qb-houses:server:getHouseDecorations', function(result)
...
function HouseKeysMenu()
    ...
    MenuTitle = "Sleutels"
    ClearMenu()
    DGCore.Functions.TriggerCallback('qb-houses:server:getHouseKeyHolders', function(holders)
        ped = PlayerPedId();
        MenuTitle = "Sleutelhouders:"
        ClearMenu()
        if holders == nil or next(holders) == nil then
            DGCore.Functions.Notify("No key holders found..", "error", 3500)
            closeMenuFull()
        else
...
RegisterNetEvent('qb-houses:client:addGarage', function()
    if closesthouse ~= nil then 
        ...
        DGCore.Functions.Notify("No house around..", "error")
    end
end)
    ...
    if dist < 1.5 then
        if hasKey then
            if Config.Houses[closesthouse].locked then
                ...
                DGCore.Functions.Notify("House is unlocked!", "success", 2500)
            else
                TriggerServerEvent('qb-houses:server:lockHouse', true, closesthouse)
                DGCore.Functions.Notify("House is locked!", "error", 2500)
            end
        else
            DGCore.Functions.Notify("You don't have the keys of the house...", "error", 3500)
        end
    else
        DGCore.Functions.Notify("There is no door nearby", "error", 3500)
    end
end)
...
RegisterNetEvent('qb-houses:client:RingDoor', function(player, house)
    if closesthouse == house and inside then
        ...
        DGCore.Functions.Notify("Someone is ringing the door!")
    end
end)
...
RegisterNetEvent('qb-houses:client:giveHouseKey', function(data)
    ...
    if player ~= -1 and distance < 2.5 and closesthouse ~= nil then
        ...
        if housedist < 10 then
            ...
            DGCore.Functions.Notify("You're not close enough to the door..", "error")
        end
    elseif closesthouse == nil then
        DGCore.Functions.Notify("There is no house near you", "error")
    else
        DGCore.Functions.Notify("No one around!", "error")
    end
end)
...
RegisterNetEvent('qb-houses:client:removeHouseKey', function(data)
    if closesthouse ~= nil then
        ...
        local housedist = #(pedpos - vector3(Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z))
        if housedist < 5 then
            DGCore.Functions.TriggerCallback('qb-houses:server:getHouseOwner', function(result)
                if DGCore.Functions.GetPlayerData().citizenid == result then
                    inHoldersMenu = true
                    HouseKeysMenu()
                    Menu.hidden = not Menu.hidden
                else
                    DGCore.Functions.Notify("You're not a house owner..", "error")
                end
            end, closesthouse)
        else
            DGCore.Functions.Notify("You're not close enough to the door..", "error")
...
RegisterNetEvent('qb-houses:client:setupHouseBlips', function()
    Citizen.CreateThread(function()
        ...
        if LocalPlayer.state['isLoggedIn'] then
            DGCore.Functions.TriggerCallback('qb-houses:server:getOwnedHouses', function(ownedHouses)
                if ownedHouses then
...
RegisterNetEvent('qb-houses:client:setLocation', function(data)
    ...
    if inside then
        if hasKey then
            ...
            DGCore.Functions.Notify('You do not own this house', 'error')
        end
    else    
        DGCore.Functions.Notify('You are not in a house', 'error')
    end
end)
...
RegisterNetEvent('qb-houses:client:HomeInvasion', function()
    ...
    if closesthouse ~= nil then
        DGCore.Functions.TriggerCallback('police:server:IsPoliceForcePresent', function(IsPresent)
            if IsPresent then
                local dist = #(pos - vector3(Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z))
                        ...
                        if not Config.Houses[closesthouse].IsRaming then
                            ...
                            Skillbar.Start({
                                ...
                                if RamsDone + 1 >= Config.RamsNeeded then
                                    TriggerServerEvent('qb-houses:server:lockHouse', false, closesthouse)
                                    DGCore.Functions.Notify('It worked the door is now out.', 'success')
                                ...
                                DGCore.Functions.Notify('It failed try again.', 'error')
                                DoRamAnimation(false)
                            end)
                            TriggerServerEvent('qb-houses:server:SetRamState', true, closesthouse)
                        else
                            DGCore.Functions.Notify('Someone is already working on the door..', 'error')
                        end
                    else
                        DGCore.Functions.Notify('19/5000 This house is already open..', 'error')
                    end
                else
                    DGCore.Functions.Notify('You\'re not near a house..', 'error')
                end
            else
                DGCore.Functions.Notify('There is no police force present..', 'error')
            end
        end)
    else
        DGCore.Functions.Notify('You\'re not near a house..', 'error')
...
RegisterNetEvent('qb-houses:client:ResetHouse', function()
    if closesthouse ~= nil then
        ...
        if Config.Houses[closesthouse].IsRammed then
            ...
            TriggerServerEvent('qb-houses:server:lockHouse', true, closesthouse)
            RamsDone = 0
            DGCore.Functions.Notify('You locked the house again..', 'success')
        else
            DGCore.Functions.Notify('This door is not broken open ..', 'error')
        end
    end
end)
...
RegisterNUICallback('HasEnoughMoney', function(data, cb)
    DGCore.Functions.TriggerCallback('qb-houses:server:HasEnoughMoney', function(hasEnough)
    end, data.objectData)
end)
