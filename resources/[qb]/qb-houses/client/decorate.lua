...
local function CheckMovementInput()
	...
	if IsControlJustReleased(0, 21) then -- Left Shift
		...
		DGCore.Functions.Notify("Speed is ".. tostring(speeds[curSpeed]))
	end
...
RegisterNetEvent('qb-houses:client:decorate', function()
	...
	if inside then
		if hasKey then
			...
			DGCore.Functions.Notify("You must have the keys to the house!", "error")
		end
	else
...
RegisterNUICallback("buySelectedObject", function(data, cb)
    DGCore.Functions.TriggerCallback('qb-houses:server:buyFurniture', function(isSuccess)
        if isSuccess then
            SetNuiFocus(false, false)
            cursorEnabled = not cursorEnabled
            SaveDecorations()
	...
	while true do
		...
		if DecoMode then
			...
			if dist > 50.0 then
				DisableEditMode()
				closeDecorateUI()
				DGCore.Functions.Notify('You have gone out of range', 'error')
			end
		end
	end
end)