...
RegisterNetEvent('DGCore:Client:OnPlayerLoaded', function()
    DGCore.Functions.TriggerCallback('qb-drugs:server:RequestConfig', function(DealerConfig)
                            ...
                            if IsControlJustPressed(0, 47) then
                                if dealer["name"] == "Mystery man" then
                                    ...
                                    if player ~= -1 and distance < 5.0 then
                                        ...
                                        DGCore.Functions.Progressbar("hospital_revive", "Put person back on their feet..", 5000, false, true, {
                                            ...
                                            DGCore.Functions.Notify("You have helped a person!")
                                        ...
                                        DGCore.Functions.Notify("There is no one nearby..", "error")
                                    end
                                else
...
function buyDealerStuff()
    ...
    for k, v in pairs(Config.Dealers[currentDealer]["products"]) do
        if DGCore.Functions.GetPlayerData().metadata["dealerrep"] >= Config.Dealers[currentDealer]["products"][k].minrep then
...
function knockDoorAnim(home)
    ...
    local myData = DGCore.Functions.GetPlayerData()
    ...
    if home then
        ...
        knockingDoor = false
        Citizen.Wait(1000)
        DGCore.Functions.Notify('It looks like no one is home..', 'error', 3500)
    end
end
...
function randomDeliveryItemOnRep()
    local ped = PlayerPedId()
    local myRep = DGCore.Functions.GetPlayerData().metadata["dealerrep"]
...
function setMapBlip(x, y)
    SetNewWaypoint(x, y)
    DGCore.Functions.Notify('The route to the delivery location has been set on your map.', 'success');
end
...
AddEventHandler('qb-drugs:client:setLocation', function(locationData)
    if activeDelivery == nil then
        ...
        DGCore.Functions.Notify('You still have an open delivery...')
        return
    end
...
function deliverStuff(activeDelivery)
    if deliveryTimeout > 0 then
        ...
        checkPedDistance()
        DGCore.Functions.Progressbar("work_dropbox", "Delivering products..", 3500, false, true, {
...
AddEventHandler('qb-drugs:client:GotoDealer', function(DealerData)
    ...
    DGCore.Functions.Notify('You have been teleported : '.. DealerData["name"] .. ' Good luck!', 'success')
end)
