...
AddEventHandler('qb-drugs:server:giveDeliveryItems', function(amount)
    ...
    TriggerClientEvent('inventory:client:ItemBox', src, DGCore.Shared.Items["weed_brick"], "add")
end)
...
AddEventHandler('qb-drugs:server:succesDelivery', function(deliveryData, inTime)
    ...
    if inTime then
        if Player.Functions.GetItemByName('weed_brick') ~= nil and Player.Functions.GetItemByName('weed_brick').amount >=
            ...
            TriggerClientEvent('inventory:client:ItemBox', src, DGCore.Shared.Items["weed_brick"], "remove")
...
DGCore.Commands.Add("newdealer", "Place A Dealer (Admin Only)", {{
    name = "name",
    help = "Dealer name"
}, {
    name = "min",
...
DGCore.Commands.Add("deletedealer", "Delete A Dealer (Admin Only)", {{
    name = "name",
    ...
    if result[1] ~= nil then
        ...
        TriggerClientEvent('DGCore:Notify', source, "Dealer: " .. dealerName .. " Has Been Deleted", "success")
    else
        TriggerClientEvent('DGCore:Notify', source, "Dealer: " .. dealerName .. " Doesn\'t Exist", "error")
    end
end, "admin")

DGCore.Commands.Add("dealers", "View All Dealers (Admin Only)", {}, false, function(source, args)
    local DealersText = ""
    if Config.Dealers ~= nil and next(Config.Dealers) ~= nil then
        ...
        TriggerClientEvent('DGCore:Notify', source, 'No dealers have been placed.', 'error')
    end
end, "admin")

DGCore.Commands.Add("dealergoto", "Teleport To A Dealer (Admin Only)", {{
    name = "name",
    help = "Dealer name"
}}, true, function(source, args)
    ...
    if Config.Dealers[DealerName] ~= nil then
        ...
        TriggerClientEvent('DGCore:Notify', source, 'This dealer doesn\'t exist.', 'error')
    end
end, "admin")
...
AddEventHandler('qb-drugs:server:CreateDealer', function(DealerData)
    ...
    if result[1] ~= nil then
        TriggerClientEvent('DGCore:Notify', src, "A dealer already exists with this name..", "error")
    else
