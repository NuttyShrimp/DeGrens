...
Citizen.CreateThread(function()
    ...
    if DGCore.Functions.GetPlayerData() ~= nil then
        ...
        DGCore.Functions.TriggerCallback('qb-traphouse:server:GetTraphousesData', function(trappies)
...
RegisterNUICallback('EnterPincode', function(d)
    ...
    if tonumber(d.pin) == data.pincode then
        ...
        DGCore.Functions.Notify('This Code Is Incorrect', 'error')
    end
end)
                        ...
                        if not IsKeyHolder then
                            ...
                            if IsHouseOwner then
                                ...
                                if IsControlJustPressed(0, 47) then
                                    DGCore.Functions.Notify('Pincode: '..data.pincode)
                                end
                            end
                            if IsControlJustPressed(0, 74) then
                                local TraphouseInventory = {}
...
AddEventHandler('qb-traphouse:client:TakeoverHouse', function(TraphouseId)
    ...
    DGCore.Functions.Progressbar("takeover_traphouse", "Taking Over", math.random(1000, 3000), false, true, {
        ...
        DGCore.Functions.Notify("Acquisitions Canceled", "error")
    end)
end)
...
function AddKeyHolder(CitizenId, Traphouse)
    if #Config.TrapHouses[Traphouse].keyholders <= 6 then
        if not HasCitizenIdHasKey(CitizenId, Traphouse) then
            ...
            DGCore.Functions.Notify(CitizenId..' Has Been Added To The Traphouse!')
        else
            DGCore.Functions.Notify(CitizenId..' This Person Already Has Keys')
        end
    else
        DGCore.Functions.Notify('You Can Give Up To 6 People Access To The Trap House!')
    end
