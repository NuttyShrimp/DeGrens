...
menu_button11:On("select",function()
    ...
    for k,v in ipairs(elements) do
        local menu_button14 = menu6:AddButton({icon = v.icon,label = v.label,value = v,description = v.description,select = function(btn)
	        ...
	        -- TODO replace with new weather sync, should be fixed after rework
...
local function CopyToClipboard(dataType)
    ...
    if dataType == 'coords3' then
        ...
        DGCore.Functions.Notify("Coordinates copied to clipboard!", "success")
    ...
    elseif dataType == 'heading' then
        ...
        DGCore.Functions.Notify("Heading copied to clipboard!", "success")
    end
end
...
local function OpenCarModelsMenu(category)
    ...
    for k, v in pairs(category) do
        local menu_button10 = menu14:AddButton({
             ...
             select = function(btn)
                 TriggerServerEvent('DGCore:CallCommand', "car", { k })
             end
        })
    end
end
...
menu12_button2:On('Select', function(item)
    TriggerServerEvent('DGCore:CallCommand', "fix", {})
end)

menu12_button3:On('Select', function(item)
    TriggerServerEvent('DGCore:CallCommand', "admincar", {})
end)

menu12_button4:On('Select', function(item)
    TriggerServerEvent('DGCore:CallCommand', "dv", {})
end)
    ...
    for k, v in ipairs(elements) do
        local menu_button10 = EditDealer:AddButton({
            ...
            select = function(btn)
                ...
                if values == "goto" then
                    TriggerServerEvent('DGCore:CallCommand', "dealergoto", { dealer["name"] })
                elseif values == "remove" then
                    TriggerServerEvent('DGCore:CallCommand', "deletedealer", { dealer["name"] })
                    EditDealer:Close()
                    menu7:Close()
                end
            end
...
menu_button4:On('Select', function(item)
    menu7:ClearItems()
    DGCore.Functions.TriggerCallback('test:getdealers', function(dealers)
...
local function OpenPermsMenu(permsply)
    DGCore.Functions.TriggerCallback('qb-admin:server:getrank', function(rank)
        if rank then
            ...
            local menu_button21 = menu10:AddButton({
                ...
                select = function(btn)
                    if selectedgroup ~= 'Unknown' then
			            ...
			            DGCore.Functions.Notify('Authority group changed!', 'success')
                        selectedgroup = 'Unknown'
                    else
                        DGCore.Functions.Notify('Choose a group!', 'error')
                    end
                end
            })
        else
    ...
    local menu_button18 = menu9:AddButton({
        ...
        select = function(btn)
            if kickreason ~= 'Unknown' then
                ...
                DGCore.Functions.Notify('You must give a reason!', 'error')
            end
        end
    })
end
    ...
    local menu_button17 = menu8:AddButton({
        ...
        select = function(btn)
            if banreason ~= 'Unknown' and banlength ~= nil then
                ...
                DGCore.Functions.Notify('You must give a Reason and set a Length for the ban!', 'error')
            end
        end
    })
end
...
menu_button2:On('select', function(item)
    menu4:ClearItems()
    DGCore.Functions.TriggerCallback('test:getplayers', function(players)
