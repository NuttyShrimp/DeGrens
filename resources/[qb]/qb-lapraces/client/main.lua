...
AddEventHandler('qb-lapraces:client:StartRaceEditor', function(RaceName)
    if not RaceData.InCreator then
        ...
        DGCore.Functions.Notify('You are already making a race.', 'error')
    end 
end)
        ...
        while RaceData.InCreator do
            ...
            if PlayerVeh ~= 0 then
                ...
                if IsControlJustPressed(0, 162) or IsDisabledControlJustPressed(0, 162) then
                    if CreatorData.Checkpoints ~= nil and next(CreatorData.Checkpoints) ~= nil then
                        ...
                        DGCore.Functions.Notify('You have not placed any checkpoints yet..', 'error')
                    end
                end
                ...
                if IsControlJustPressed(0, 311) or IsDisabledControlJustPressed(0, 311) then
                    if CreatorData.Checkpoints ~= nil and #CreatorData.Checkpoints >= 2 then
                        ...
                        DGCore.Functions.Notify('You must have at least 10 checkpoints', 'error')
                    end
                end
                ...
                if IsControlJustPressed(0, 40) or IsDisabledControlJustPressed(0, 40) then
                    if CreatorData.TireDistance + 1.0 ~= 16.0 then
                        ...
                        DGCore.Functions.Notify('You can not go higher than 15')
                    end
                end
                ...
                if IsControlJustPressed(0, 39) or IsDisabledControlJustPressed(0, 39) then
                    if CreatorData.TireDistance - 1.0 ~= 1.0 then
                        ...
                        DGCore.Functions.Notify('You cannot go lower than 2')
                    end
                end
            else
            ...
            if IsControlJustPressed(0, 163) or IsDisabledControlJustPressed(0, 163) then
                if not CreatorData.ConfirmDelete then
                    ...
                    DGCore.Functions.Notify('Press [9] again to confirm', 'error', 5000)
                else
                    ...
                    DGCore.Functions.Notify('Race-editor canceled!', 'error')
...
function SaveRace()
    ...
    DGCore.Functions.Notify('Race: '..CreatorData.RaceName..' is saved!', 'success')
...
function DeleteCheckpoint()
    ...
    if RaceData.ClosestCheckpoint ~= 0 then
        if CreatorData.Checkpoints[RaceData.ClosestCheckpoint] ~= nil then
            ...
            DGCore.Functions.Notify('You cant go to fast', 'error')
        end
    else
        DGCore.Functions.Notify('You cant go too fast', 'error')
    end
end
...
AddEventHandler('qb-lapraces:client:JoinRace', function(Data, Laps)
    if not RaceData.InRace then
        ...
        DGCore.Functions.Notify('Youre already in a race..', 'error')
    end
end)

RegisterNetEvent('qb-lapraces:client:LeaveRace')
AddEventHandler('qb-lapraces:client:LeaveRace', function(data)
    DGCore.Functions.Notify('You have completed the race!')
    for k, v in pairs(CurrentRaceData.Checkpoints) do
...
AddEventHandler('qb-lapraces:client:RaceCountdown', function()
    ...
    if CurrentRaceData.RaceId ~= nil then
        while Countdown ~= 0 do
            if CurrentRaceData.RaceName ~= nil then
                if Countdown == 10 then
                    DGCore.Functions.Notify('The race will start in 10 seconds', 'error', 2500)
                    PlaySound(-1, "slow", "SHORT_PLAYER_SWITCH_SOUND_SET", 0, 0, 1)
                elseif Countdown <= 5 then
                    DGCore.Functions.Notify(Countdown, 'error', 500)
                    PlaySound(-1, "slow", "SHORT_PLAYER_SWITCH_SOUND_SET", 0, 0, 1)
                end
        ...
        if CurrentRaceData.RaceName ~= nil then
            ...
            DGCore.Functions.Notify('GO!', 'success', 1000)
        ...
        DGCore.Functions.Notify('You are not currently in a race..', 'error')
    end
end)
...
function FinishRace()
    ...
    if CurrentRaceData.BestLap ~= 0 then
        DGCore.Functions.Notify('Race finished in '..SecondsToClock(CurrentRaceData.TotalTime)..', with the best lap: '..SecondsToClock(CurrentRaceData.BestLap))
    else
        DGCore.Functions.Notify('Race finished in '..SecondsToClock(CurrentRaceData.TotalTime))
    end
    for k, v in pairs(CurrentRaceData.Checkpoints) do
        if CurrentRaceData.Checkpoints[k].blip ~= nil then
            RemoveBlip(CurrentRaceData.Checkpoints[k].blip)
...
AddEventHandler('qb-lapraces:client:PlayerFinishs', function(RaceId, Place, FinisherData)
    if CurrentRaceData.RaceId ~= nil then
        if CurrentRaceData.RaceId == RaceId then
            DGCore.Functions.Notify(FinisherData.PlayerData.charinfo.firstname..' is finished on spot: '..Place, 'error', 3500)
        end
    end
end)
                ...
                if CurrentRaceData.Checkpoints[1] ~= nil then
                    ...
                    if dist > 115.0 then
                        if ToFarCountdown ~= 0 then
                            ...
                            DGCore.Functions.Notify('Go back to the start or you will be kicked from the race: '..ToFarCountdown..'s', 'error', 500)
                        else
