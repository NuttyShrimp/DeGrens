...
AddEventHandler('DGCore:Client:OnPlayerLoaded', function()
    ...
    isLoggedIn = true
    DGCore.Functions.TriggerCallback('qb-vehicletuning:server:GetAttachedVehicle', function(plates)
        for k, v in pairs(plates) do
            Config.Plates[k].AttachedVehicle = v.AttachedVehicle
        end
    end)

    DGCore.Functions.TriggerCallback('qb-vehicletuning:server:GetDrivingDistances', function(retval)
...
RegisterNetEvent('DGCore:Client:SetDuty')
AddEventHandler('DGCore:Client:SetDuty', function(duty)
    onDuty = duty
end)
                                ...
                                if PlateDistance < 2 then
                                    ...
                                    if IsPedInAnyVehicle(PlayerPedId()) then
                                        if not IsThisModelABicycle(GetEntityModel(veh)) then
                                            ...
                                            DGCore.Functions.Notify("You Cannot Put Bicycles On The Platform!", "error")
                                        end
                                    end
                                end
                            end
...
function SpawnListVehicle(model)
    ...
    DGCore.Functions.SpawnVehicle(model, function(veh)
...
function PartMenu(part)
    ClearMenu()
    Menu.addButton("Repair ("..DGCore.Shared.Items[Config.RepairCostAmount[part].item]["label"].." "..Config.RepairCostAmount[part].costs.."x)", "RepairPart", part)
    Menu.addButton("Back", "VehicleOptions", nil)
    Menu.addButton("Close Menu", "CloseMenu", nil) 
end
...
function RepairPart(part)
    ...
    DGCore.Functions.TriggerCallback('qb-inventory:server:GetStashItems', function(StashItems)
        ...
        if hasitem and countitem >= PartData.costs then
            ...
            DGCore.Functions.Progressbar("repair_part", "Repairing " ..Config.ValuesLabels[part], math.random(5000, 10000), false, true, {
                ...
                DGCore.Functions.Notify("Repair Cancelled", "error")
            end)
        else
            DGCore.Functions.Notify('There Are Not Enough Materials In The Safe', 'error')
        end
    end, "mechanicstash")
end
...
AddEventHandler('qb-vehicletuning:client:RepaireeePart', function(part)
    ...
    DGCore.Functions.Notify("The "..Config.ValuesLabels[part].." Is Repaired!")
end)
            ...
            if #(pos - vehpos) < 5.0 then
                if not IsThisModelABicycle(GetEntityModel(veh)) then
                    ...
                    if VehicleStatus[plate] ~= nil then 
                        ...
                        DGCore.Functions.Notify("Status Unknown", "error")
                    end
                else
                    DGCore.Functions.Notify("Not A Valid Vehicle", "error")
                end
            else
                DGCore.Functions.Notify("You Are Not Close Enough To The Vehicle", "error")
            end
        else
            DGCore.Functions.Notify("You Must Be In The Vehicle First", "error")
        end
    else
        DGCore.Functions.Notify("You Must Be Outside The Vehicle", "error")
    end
end)
...
AddEventHandler('vehiclemod:client:fixEverything', function()
    if (IsPedInAnyVehicle(PlayerPedId(), false)) then
        ...
        if not IsThisModelABicycle(GetEntityModel(veh)) and GetPedInVehicleSeat(veh, -1) == PlayerPedId() then
            ...
            DGCore.Functions.Notify("You Are Not The Driver Or On A Bicycle", "error")
        end
    else
...
AddEventHandler('vehiclemod:client:repairPart', function(part, level, needAmount)
    -- if CanReapair() then
        if not IsPedInAnyVehicle(PlayerPedId(), false) then
                ...
                if #(pos - vehpos) < 5.0 then
                    if not IsThisModelABicycle(GetEntityModel(veh)) then
                        ...
                        if VehicleStatus[plate] ~= nil and VehicleStatus[plate][part] ~= nil then
                            ...
                            DGCore.Functions.Progressbar("repair_advanced", "Repair Vehicle", lockpickTime, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                                ...
                                if part == "body" then
                                    ...
                                    TriggerEvent("inventory:client:ItemBox", DGCore.Shared.Items[Config.RepairCost[part]], "remove")
                            ...
                            DGCore.Functions.Notify("Not A Valid Part", "error")
                        end
                    else
                        DGCore.Functions.Notify("Not A Valid Vehicle", "error")
                    end
            ...
            DGCore.Functions.Notify("Youre Not In a Vehicle", "error")
        end
    -- end
end)
...
function SendStatusMessage(statusList)
    if statusList ~= nil then 
        TriggerEvent('chat:addMessage', {
            ...
            args = {'Vehicle Status', round(statusList["engine"]) .. "/" .. Config.MaxStatusValues["engine"] .. " ("..DGCore.Shared.Items["advancedrepairkit"]["label"]..")", round(statusList["body"]) .. "/" .. Config.MaxStatusValues["body"] .. " ("..DGCore.Shared.Items[Config.RepairCost["body"]]["label"]..")", round(statusList["radiator"]) .. "/" .. Config.MaxStatusValues["radiator"] .. ".0 ("..DGCore.Shared.Items[Config.RepairCost["radiator"]]["label"]..")", round(statusList["axle"]) .. "/" .. Config.MaxStatusValues["axle"] .. ".0 ("..DGCore.Shared.Items[Config.RepairCost["axle"]]["label"]..")", round(statusList["brakes"]) .. "/" .. Config.MaxStatusValues["brakes"] .. ".0 ("..DGCore.Shared.Items[Config.RepairCost["brakes"]]["label"]..")", round(statusList["clutch"]) .. "/" .. Config.MaxStatusValues["clutch"] .. ".0 ("..DGCore.Shared.Items[Config.RepairCost["clutch"]]["label"]..")", round(statusList["fuel"]) .. "/" .. Config.MaxStatusValues["fuel"] .. ".0 ("..DGCore.Shared.Items[Config.RepairCost["fuel"]]["label"]..")"}
        })
    end
end
