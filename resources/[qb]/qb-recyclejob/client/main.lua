        ...
        if #(pos - vector3(1049.15, -3100.63, -39.95)) < 15 and not IsPedInAnyVehicle(PlayerPedId(), false) and carryPackage == nil then
            ...
            if #(pos - vector3(1049.15, -3100.63, -39.95)) < 1.3 then
                ...
                if IsControlJustReleased(0, 38) then
                    onDuty = not onDuty
                    if onDuty then
                        DGCore.Functions.Notify("You Have Been Clocked In", "success")
                    else
                        DGCore.Functions.Notify("You Have Clocked Out", "error")
                    end
                end
            end
        end
            ...
            if packagePos ~= nil then
                ...
                if carryPackage == nil then
                    if #(pos - vector3(packagePos.x, packagePos.y, packagePos.z)) < 2.3 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            DGCore.Functions.Progressbar("pickup_reycle_package", "Pick Up The Package ..", math.random(4000, 6000), false, true, {
                    ...
                    if #(pos - vector3(Config['delivery'].dropLocation.x, Config['delivery'].dropLocation.y, Config['delivery'].dropLocation.z)) < 2.0 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            DropPackage()
                            ScrapAnim()
                            DGCore.Functions.Progressbar("deliver_reycle_package", "Unpacking The Package", 5000, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
