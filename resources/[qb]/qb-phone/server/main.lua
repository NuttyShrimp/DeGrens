...
function GetOnlineStatus(number)
    local Target = DGCore.Functions.GetPlayerByPhone(number)
    local retval = false
    if Target ~= nil then
        retval = true
    end
...
DGCore.Functions.CreateCallback('qb-phone:server:GetPhoneData', function(source, cb)
    local src = source
    local Player = DGCore.Functions.GetPlayer(src)
    if Player ~= nil then
        local PhoneData = {
        ...
        if invoices[1] ~= nil then
            for k, v in pairs(invoices) do
                local Ply = DGCore.Functions.GetPlayerByCitizenId(v.sender)
                if Ply ~= nil then
        ...
        if garageresult[1] ~= nil then
            for k, v in pairs(garageresult) do
                ...
                if (DGCore.Shared.Vehicles[vehicleModel] ~= nil) and (Garages[v.garage] ~= nil) then
                    v.garage = Garages[v.garage].label
                    v.vehicle = DGCore.Shared.Vehicles[vehicleModel].name
                    v.brand = DGCore.Shared.Vehicles[vehicleModel].brand
                end
...
DGCore.Functions.CreateCallback('qb-phone:server:GetCallState', function(source, cb, ContactData)
    local Target = DGCore.Functions.GetPlayerByPhone(ContactData.number)
...
AddEventHandler('qb-phone:server:CallContact', function(TargetData, CallId, AnonymousCall)
    local src = source
    local Ply = DGCore.Functions.GetPlayer(src)
    local Target = DGCore.Functions.GetPlayerByPhone(TargetData.number)
...
AddEventHandler('qb-phone:server:BillingEmail', function(data, paid)
    for k, v in pairs(DGCore.Functions.GetPlayers()) do
        local target = DGCore.Functions.GetPlayer(v)
        if target.PlayerData.job.name == data.society then
            if paid then
                local name = '' .. DGCore.Functions.GetPlayer(source).PlayerData.charinfo.firstname .. ' ' ..
                                 DGCore.Functions.GetPlayer(source).PlayerData.charinfo.lastname .. ''
...
DGCore.Functions.CreateCallback('qb-phone:server:PayInvoice', function(source, cb, society, amount, invoiceId, sendercitizenid)
    local Invoices = {}
    local Ply = DGCore.Functions.GetPlayer(source)
    local SenderPly = DGCore.Functions.GetPlayerByCitizenId(sendercitizenid)
...
DGCore.Functions.CreateCallback('qb-phone:server:DeclineInvoice', function(source, cb, sender, amount, invoiceId)
    local Invoices = {}
    local Ply = DGCore.Functions.GetPlayer(source)
    exports.oxmysql:execute('DELETE FROM phone_invoices WHERE id = ?', {invoiceId})
    local invoices = exports.oxmysql:executeSync('SELECT * FROM phone_invoices WHERE citizenid = ?',
...
DGCore.Commands.Add('bill', 'Bill A Player', {{
    name = 'id',
    help = 'Player ID'
}, {
    name = 'amount',
    ...
    local biller = DGCore.Functions.GetPlayer(source)
    local billed = DGCore.Functions.GetPlayer(tonumber(args[1]))
        ...
        if billed ~= nil then
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    ...
                    TriggerClientEvent('DGCore:Notify', source, 'Invoice Successfully Sent', 'success')
                    TriggerClientEvent('DGCore:Notify', billed.PlayerData.source, 'New Invoice Received')
                else
                    TriggerClientEvent('DGCore:Notify', source, 'Must Be A Valid Amount Above 0', 'error')
                end
            else
                TriggerClientEvent('DGCore:Notify', source, 'You Cannot Bill Yourself', 'error')
            end
        else
            TriggerClientEvent('DGCore:Notify', source, 'Player Not Online', 'error')
        end
    else
        TriggerClientEvent('DGCore:Notify', source, 'No Access', 'error')
    end
end)
...
DGCore.Functions.CreateCallback('qb-phone:server:GetContactPictures', function(source, cb, Chats)
    for k, v in pairs(Chats) do
        local Player = DGCore.Functions.GetPlayerByPhone(v.number)
...
DGCore.Functions.CreateCallback('qb-phone:server:GetContactPicture', function(source, cb, Chat)
    local Player = DGCore.Functions.GetPlayerByPhone(Chat.number)
...
DGCore.Functions.CreateCallback('qb-phone:server:GetPicture', function(source, cb, number)
...
AddEventHandler('qb-phone:server:SetPhoneAlerts', function(app, alerts)
    local src = source
    local CitizenId = DGCore.Functions.GetPlayer(src).citizenid
...
AddEventHandler('qb-phone:server:TransferMoney', function(iban, amount)
    local src = source
    local sender = DGCore.Functions.GetPlayer(src)

    local query = '%' .. iban .. '%'
    local result = exports.oxmysql:executeSync('SELECT * FROM players WHERE charinfo LIKE ?', {query})
    if result[1] ~= nil then
        local reciever = DGCore.Functions.GetPlayerByCitizenId(result[1].citizenid)
        ...
        TriggerClientEvent('DGCore:Notify', src, "This account number doesn't exist!", "error")
    end
end)
...
AddEventHandler('qb-phone:server:UpdateMessages', function(ChatMessages, ChatNumber, New)
    local src = source
    local SenderData = DGCore.Functions.GetPlayer(src)

    local query = '%' .. ChatNumber .. '%'
    local Player = exports.oxmysql:executeSync('SELECT * FROM players WHERE charinfo LIKE ?', {query})
    if Player[1] ~= nil then
        local TargetData = DGCore.Functions.GetPlayerByCitizenId(Player[1].citizenid)
...
AddEventHandler('qb-phone:server:AddRecentCall', function(type, data)
    ...
    local Trgt = DGCore.Functions.GetPlayerByPhone(data.number)
    if Trgt ~= nil then
...
AddEventHandler('qb-phone:server:CancelCall', function(ContactData)
    local Ply = DGCore.Functions.GetPlayerByPhone(ContactData.TargetData.number)
...
AddEventHandler('qb-phone:server:AnswerCall', function(CallData)
    local Ply = DGCore.Functions.GetPlayerByPhone(CallData.TargetData.number)
...
DGCore.Functions.CreateCallback('qb-phone:server:FetchResult', function(source, cb, search)
    local src = source
...
DGCore.Functions.CreateCallback('qb-phone:server:GetVehicleSearchResults', function(source, cb, search)
    local src = source
    local search = escape_sqli(search)
    local searchData = {}
    local query = '%' .. search .. '%'
    ...
    if result[1] ~= nil then
        for k, v in pairs(result) do
            ...
            if player[1] ~= nil then
                ...
                local vehicleInfo = DGCore.Shared.Vehicles[result[k].vehicle]
...
DGCore.Functions.CreateCallback('qb-phone:server:ScanPlate', function(source, cb, plate)
    local src = source
    local vehicleData = {}
    if plate ~= nil then
        ...
        TriggerClientEvent('DGCore:Notify', src, 'No Vehicle Nearby', 'error')
        cb(nil)
    end
end)
...
DGCore.Functions.CreateCallback('qb-phone:server:GetGarageVehicles', function(source, cb)
    ...
    if result[1] ~= nil then
        for k, v in pairs(result) do
            local VehicleData = DGCore.Shared.Vehicles[v.vehicle]
...
DGCore.Functions.CreateCallback('qb-phone:server:HasPhone', function(source, cb)
    local Player = DGCore.Functions.GetPlayer(source)
...
DGCore.Functions.CreateCallback('qb-phone:server:CanTransferMoney', function(source, cb, amount, iban)
    local Player = DGCore.Functions.GetPlayer(source)
    ...
    if (Player.PlayerData.money.bank - amount) >= 0 then
        ...
        if result[1] ~= nil then
            ...
            TriggerClientEvent('DGCore:Notify', source, "This account number does not exist!", "error")
            cb(false)
        end
    end
end)
...
DGCore.Functions.CreateCallback('qb-phone:server:GetCurrentLawyers', function(source, cb)
    local Lawyers = {}
    for k, v in pairs(DGCore.Functions.GetPlayers()) do
        local Player = DGCore.Functions.GetPlayer(v)
        if Player ~= nil then
...
DGCore.Commands.Add("setmetadata", "Set Player Metadata (God Only)", {}, false, function(source, args)
    local Player = DGCore.Functions.GetPlayer(source)
