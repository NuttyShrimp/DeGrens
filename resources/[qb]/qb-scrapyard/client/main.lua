							...
							if IsControlJustReleased(0, 38) then
								if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() then
									if IsVehicleValid(GetEntityModel(vehicle)) then 
										...
										DGCore.Functions.TriggerCallback('qb-scrapyard:checkOwnerVehicle',function(retval)
											if retval then 
												ScrapVehicle(vehicle)
											else
												DGCore.Functions.Notify("You can't smash a vehicle that owns it.", "error")
											end
										end,vehiclePlate)
									else
										DGCore.Functions.Notify("This Vehicle Cannot Be Scrapped.", "error")
									end
								else
									DGCore.Functions.Notify("You Are Not The Driver", "error")
								end
							end
						end
					end
	...
	if Config.CurrentVehicles ~= nil and next(Config.CurrentVehicles) ~= nil then 
		...
		for k, v in pairs(Config.CurrentVehicles) do
			if Config.CurrentVehicles[k] ~= nil then 
				local vehicleInfo = DGCore.Shared.Vehicles[v]
...
function ScrapVehicle(vehicle)
	...
	DGCore.Functions.Progressbar("scrap_vehicle", "Demolish Vehicle", scrapTime, false, true, {
