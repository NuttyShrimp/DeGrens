...
AddEventHandler('inventory:server:combineItem', function(item, fromItem, toItem)
	...
	local recipe = DGCore.Shared.Items[toItem.name].combinable
			...
			if name == "stash" then
				if Stashes[id] ~= nil then
					if Stashes[id].isOpen then
						local Target = DGCore.Functions.GetPlayer(Stashes[id].isOpen)
						if Target ~= nil then
			...
			elseif name == "trunk" then
				if Trunks[id] ~= nil then
					if Trunks[id].isOpen then
						local Target = DGCore.Functions.GetPlayer(Trunks[id].isOpen)
						if Target ~= nil then
				...
				if (Trunks[id] ~= nil and Trunks[id].isOpen) or (DGCore.Shared.SplitStr(id, "PLZI")[2] ~= nil and Player.PlayerData.job.name ~= "police") then
					secondInv.name = "none-inv"
			...
			elseif name == "glovebox" then
				if Gloveboxes[id] ~= nil then
					if Gloveboxes[id].isOpen then
						local Target = DGCore.Functions.GetPlayer(Gloveboxes[id].isOpen)
						if Target ~= nil then
			...
			elseif name == "otherplayer" then
				...
				if OtherPlayer ~= nil then
					...
					if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
						...
						secondInv.slots = DGCore.Config.Player.MaxInvSlots - 1
					end
					Citizen.Wait(250)
				end
			else
...
AddEventHandler('inventory:server:UseItemSlot', function(slot)
	...
	if itemData ~= nil then
		local itemInfo = DGCore.Shared.Items[itemData.name]
...
AddEventHandler('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
	...
	if (fromInventory == "player" or fromInventory == "hotbar") and (DGCore.Shared.SplitStr(toInventory, "-")[1] == "itemshop" or toInventory == "crafting") then
		return
	end
	...
	if fromInventory == "player" or fromInventory == "hotbar" then
		...
		if fromItemData ~= nil and fromItemData.amount >= fromAmount then
			...
			elseif DGCore.Shared.SplitStr(toInventory, "-")[1] == "otherplayer" then
				local playerId = tonumber(DGCore.Shared.SplitStr(toInventory, "-")[2])
				...
				if toItemData ~= nil then
					--Player.PlayerData.items[fromSlot] = toItemData
					local itemInfo = DGCore.Shared.Items[toItemData.name:lower()]
					local toAmount = tonumber(toAmount) ~= nil and tonumber(toAmount) or toItemData.amount
					if toItemData.name ~= fromItemData.name then
					...
					local itemInfo = DGCore.Shared.Items[fromItemData.name:lower()]
			...
			elseif DGCore.Shared.SplitStr(toInventory, "-")[1] == "trunk" then
				local plate = DGCore.Shared.SplitStr(toInventory, "-")[2]
			...
			elseif DGCore.Shared.SplitStr(toInventory, "-")[1] == "glovebox" then
				local plate = DGCore.Shared.SplitStr(toInventory, "-")[2]
			...
			elseif DGCore.Shared.SplitStr(toInventory, "-")[1] == "stash" then
				local stashId = DGCore.Shared.SplitStr(toInventory, "-")[2]
			...
			elseif DGCore.Shared.SplitStr(toInventory, "-")[1] == "traphouse" then
				-- Traphouse
				local traphouseId = DGCore.Shared.SplitStr(toInventory, "-")[2]
				...
				if IsItemValid then
					...
					TriggerClientEvent('DGCore:Notify', src, "You can\'t sell this item..", 'error')
				end
			else
				-- drop
	...
	elseif DGCore.Shared.SplitStr(fromInventory, "-")[1] == "otherplayer" then
		local playerId = tonumber(DGCore.Shared.SplitStr(fromInventory, "-")[2])
		local OtherPlayer = DGCore.Functions.GetPlayer(playerId)
	...
	elseif DGCore.Shared.SplitStr(fromInventory, "-")[1] == "trunk" then
		local plate = DGCore.Shared.SplitStr(fromInventory, "-")[2]
	...
	elseif DGCore.Shared.SplitStr(fromInventory, "-")[1] == "glovebox" then
		local plate = DGCore.Shared.SplitStr(fromInventory, "-")[2]
	...
	elseif DGCore.Shared.SplitStr(fromInventory, "-")[1] == "stash" then
		local stashId = DGCore.Shared.SplitStr(fromInventory, "-")[2]
	...
	elseif DGCore.Shared.SplitStr(fromInventory, "-")[1] == "traphouse" then
		local traphouseId = DGCore.Shared.SplitStr(fromInventory, "-")[2]
	...
	elseif DGCore.Shared.SplitStr(fromInventory, "-")[1] == "itemshop" then
		...
		local itemInfo = DGCore.Shared.Items[itemData.name:lower()]
		local bankBalance = Player.PlayerData.money["bank"]
		local price = tonumber((itemData.price*fromAmount))

		if DGCore.Shared.SplitStr(shopType, "_")[1] == "Dealer" then
			if DGCore.Shared.SplitStr(itemData.name, "_")[1] == "weapon" then
				...
				if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
					...
					TriggerClientEvent('DGCore:Notify', src, "You don\'t have enough cash..", "error")
				end
			else
				if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
		...
		elseif DGCore.Shared.SplitStr(shopType, "_")[1] == "Itemshop" then
	...
	elseif fromInventory == "crafting" then
		...
		if hasCraftItems(src, itemData.costs, fromAmount) then
			...
			TriggerClientEvent('DGCore:Notify', src, "You don't have the right items..", "error")
		end
...
DGCore.Functions.CreateCallback('qb-inventory:server:GetStashItems', function(source, cb, stashId)
...
function AddToStash(stashId, slot, otherslot, itemName, amount, info)
	local amount = tonumber(amount)
	local ItemData = DGCore.Shared.Items[itemName]
...
DGCore.Commands.Add("trunkpos", "View Trunk Location", {}, false, function(source, args)
	TriggerClientEvent("inventory:client:ShowTrunkPos", source)
end)

DGCore.Commands.Add("rob", "Rob Player", {}, false, function(source, args)
...
DGCore.Commands.Add("giveitem", "Give An Item (Admin Only)", {{name="id", help="Player ID"},{name="item", help="Name of the item (not a label)"}, {name="amount", help="Amount of items"}}, true, function(source, args)
	...
	local itemData = DGCore.Shared.Items[tostring(args[2]):lower()]
	if Player ~= nil then
		if amount > 0 then
			if itemData ~= nil then
				-- check iteminfo
...
DGCore.Commands.Add("randomitems", "Give Random Items (God Only)", {}, false, function(source, args)
	local Player = DGCore.Functions.GetPlayer(source)
	local filteredItems = {}
	for k, v in pairs(DGCore.Shared.Items) do
		if DGCore.Shared.Items[k]["type"] ~= "weapon" then
	...
	for i = 1, 10, 1 do
		...
		if Player.Functions.AddItem(randitem["name"], amount) then
			TriggerClientEvent('inventory:client:ItemBox', source, DGCore.Shared.Items[randitem["name"]], 'add')
            Citizen.Wait(500)
		end
	end
end, "god")

DGCore.Functions.CreateUseableItem("snowball", function(source, item)
	local Player = DGCore.Functions.GetPlayer(source)
	local itemData = Player.Functions.GetItemBySlot(item.slot)
