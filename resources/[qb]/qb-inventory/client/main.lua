    ...
    if bool then
        for k, v in pairs(items) do
            table.insert(itemTable, {
                item = items[k].name,
                label = DGCore.Shared.Items[items[k].name]["label"],
                image = items[k].image,
            })
        end
    end
...
AddEventHandler("inventory:client:UpdatePlayerInventory", function(isError)
    SendNUIMessage({
        action = "update",
        inventory = DGCore.Functions.GetPlayerData().items,
        maxweight = DGCore.Config.Player.MaxWeight,
        slots = MaxInventorySlots,
        error = isError,
    })
...
AddEventHandler("inventory:client:CraftItems", function(itemName, itemCosts, amount, toSlot, points)
    ...
    isCrafting = true
    DGCore.Functions.Progressbar("repair_vehicle", "Crafting..", (math.random(2000, 5000) * amount), false, true, {
        ...
        TriggerEvent('inventory:client:ItemBox', DGCore.Shared.Items[itemName], 'add')
        isCrafting = false
	end, function() -- Cancel
		StopAnimTask(ped, "mini@repair", "fixing_a_player", 1.0)
...
AddEventHandler("inventory:client:PickupSnowballs", function()
    ...
    DGCore.Functions.Progressbar("pickupsnowball", "Collecting snowballs..", 1500, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        ClearPedTasks(ped)
        TriggerServerEvent('DGCore:Server:AddItem', "snowball", 1)
        TriggerEvent('inventory:client:ItemBox', DGCore.Shared.Items["snowball"], "add")
    end, function() -- Cancel
        ClearPedTasks(ped)
...
RegisterNUICallback('GetWeaponData', function(data, cb)
    local data = {
        WeaponData = DGCore.Shared.Items[data.weapon],
...
RegisterNUICallback('RemoveAttachment', function(data, cb)
    local ped = PlayerPedId()
    local WeaponData = DGCore.Shared.Items[data.WeaponData.name]
...
RegisterNUICallback('getCombineItem', function(data, cb)
    cb(DGCore.Shared.Items[data.item])
end)
...
function ToggleHotbar(toggle)
    local HotbarItems = {
        [1] = DGCore.Functions.GetPlayerData().items[1],
        [2] = DGCore.Functions.GetPlayerData().items[2],
        [3] = DGCore.Functions.GetPlayerData().items[3],
        [4] = DGCore.Functions.GetPlayerData().items[4],
        [5] = DGCore.Functions.GetPlayerData().items[5],
        [41] = DGCore.Functions.GetPlayerData().items[41],
    } 
