...
Citizen.CreateThread(function()
    Wait(1000)
    if DGCore.Functions.GetPlayerData().job ~= nil and next(DGCore.Functions.GetPlayerData().job) then
                            ...
                            if IsControlJustPressed(0, 38) then
                                if CurrentCops >= Config.MinimumStoreRobberyPolice then
                                    ...
                                    if Config.Safes[safe].type == "keypad" then
                                        ...
                                        DGCore.Functions.TriggerCallback('qb-storerobbery:server:getPadlockCombination', function(combination)
                                    ...
                                    DGCore.Functions.Notify("Not Enough Police (".. Config.MinimumStoreRobberyPolice .." Required)", "error")
                                end
                            end
                        else
    ...
    for k, v in pairs(Config.Registers) do
        ...
        if dist <= 1 and not Config.Registers[k].robbed then
            if CurrentCops >= Config.MinimumStoreRobberyPolice then
                ...
                DGCore.Functions.Notify("Not Enough Police (2 Required)", "error")
            end
        end
    end
end)
...
function setupRegister()
    DGCore.Functions.TriggerCallback('qb-storerobbery:server:getRegisterStatus', function(Registers)
...
function setupSafes()
    DGCore.Functions.TriggerCallback('qb-storerobbery:server:getSafeStatus', function(Safes)
...
RegisterNUICallback('success', function()
    if currentRegister ~= 0 then
        ...
        DGCore.Functions.Progressbar("search_register", "Emptying The Register..", lockpickTime, false, true, {
...
RegisterNUICallback('fail', function()
    ...
    if (IsWearingHandshoes() and math.random(1, 100) <= 25) then
        local pos = GetEntityCoords(PlayerPedId())
        TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
        DGCore.Functions.Notify("You Broke The Lock Pick")
    end
    lockpick(false)
end)
...
RegisterNUICallback('TryCombination', function(data, cb)
    DGCore.Functions.TriggerCallback('qb-storerobbery:server:isCombinationRight', function(combination)
