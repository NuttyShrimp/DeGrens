                ...
                if dist < 10 then
                    ...
                    if dist < 1 then
                        ...
                        if IsControlJustPressed(0, 38) then -- E
                            local ShopItems = {}
                            ShopItems.items = {}
                            DGCore.Functions.TriggerCallback('qb-shops:server:getLicenseStatus', function(hasLicense, hasLicenseItem)
                                ShopItems.label = Config.Locations[shop]["label"]
                                if Config.Locations[shop].type == "weapon" then
                                    if hasLicense and hasLicenseItem then
                                        ShopItems.items = SetupItems(shop)
                                        DGCore.Functions.Notify("The dealer verifies your license", "success")
                                        Citizen.Wait(500)
                                    else
                                        for i = 1, #products do
                                            if not products[i].requiredJob then
                                                ...
                                                for i2 = 1, #products[i].requiredJob do
                                                    if DGCore.Functions.GetPlayerData().job.name == products[i].requiredJob[i2] and not products[i].requiresLicense then
                                                        table.insert(ShopItems.items, products[i])
                                                    end
                                                end
                                            end
                                        end
                                        DGCore.Functions.Notify("The dealer declines to show you firearms", "error")
                                        Citizen.Wait(500)
                                        DGCore.Functions.Notify("Speak with law enforcement to get a firearms license", "error")
                                        Citizen.Wait(1000)
                                    end
                                else
                                    ShopItems.items = SetupItems(shop)
