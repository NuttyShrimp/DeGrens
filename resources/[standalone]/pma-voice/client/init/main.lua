...
---@param origin "me" | "other" who initiated the micclick
function playMicClicks(clickType, origin)
  if not radioConfig or not radioConfig.clicks or radioConfig.clicks.enabled == false then
    return
  end
  local clickKey = origin == 'me' and 'me' or 'someElse'
  local clickWay = clickType and 'incoming' or 'outgoing'
  if radioConfig.clicks[clickKey] and radioConfig.clicks[clickKey][clickWay] == false then
    return
  end
